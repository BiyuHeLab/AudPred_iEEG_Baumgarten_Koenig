function NASTD_ECoG_Predict_CompPredp33_Subs(sub,  tonedur_text, predictive_sequencerange, toneIndex)
...
ECoGdata_p33 = zeros(nSensors, length(t_start_roundind(1):t_end_roundind(1)), nTrials); %create empty prox
    ...
    for i_win = 1:size(windows,1)
        ...
        for i_sensor = 1:nSensors
            ...
            if predictive_sequencerange(i_k) > 1 %only if the currently focused tone is not the first tone (since for first tone we can't really predict anything)
                ...
                dv = ERF_p33_win{i_win}(i_sensor,:)';  % mean ERF window at this sensor for all trials
...
%             stats = regstats(dv, iv_pred_error, 'linear');
            ...
            simplepred_err_t1{i_k}{i_win}(i_sensor) = stats.tstat.t(2);
            simplepred_err_t1_stats{i_k}{i_win}{i_sensor} = stats;
            simplepred_err_t1_p{i_k}{i_win}(i_sensor) = stats.tstat.pval(2);
...
%% 8) Save variables
mkdir([path_save]);
savefile = [path_save sub '_NEWPredictionERF_regstatsEXP_' BC_text '_' tonedur_title 'msToneDur.mat'];

save(savefile, 'pred_t1', 'pred_t1_stats', 'pred_t1_p', ... %'pred_t2', 'pred_t2_stats', 'pred_t2_p', ...
               'pred_err_t1', 'pred_err_t1_stats', 'pred_err_t1_p', ...
               'simplepred_err_t1', 'simplepred_err_t1_stats', 'simplepred_err_t1_p', ...               
