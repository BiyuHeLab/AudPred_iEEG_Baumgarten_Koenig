...
for i_effect = 1:length(param.GC.ElecPairEffect)
    ...
    %2.1 Read out detailed electrode label and correspoding anatomical region label
    ...
    for i_sub = 1:length(subs)
        if ~isempty(GCdata{i_sub}.temporalGC{i_effect})
            ...
            for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                ...
                temp_eleclabel_sourceelec = GCdata{i_sub}.label_allelecs{GCdata{i_sub}.ind_pairedelecs_from{i_effect}(i_sourceelec)};
                ...
                temp_anatlabel_sourceelec = fields(SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(param.GC.ElecPairRegion).Pairs_perelec);
                ...
                temp_num_targetelecs = size(SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
                ...
                for i_targetelec = 1:temp_num_targetelecs
                    fulllabel_targetelec{i_sub}{i_targetelec, i_sourceelec} = ...
                        cell2mat(strcat(SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
                        param.GC.ElecPairRegion).Pairs_perelec.(...
                    ...
                    anatlabel_targetelec{i_sub}{i_targetelec, i_sourceelec} = ...
                        cell2mat(SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
                        param.GC.ElecPairRegion).Pairs_perelec.(...
                        temp_anatlabel_sourceelec){i_targetelec,5});
                    anatlabel_targetelec{i_sub}{i_targetelec, i_sourceelec} = ...
    ...
    %Create filter limiting to-be-analyzed anatomical regions to the ones
    %selected
    ...
    %2.3 Define anatreg index and label for each source and target elec per subject
    for i_sub = 1:length(subs)
        if ~isempty(GCdata{i_sub}.temporalGC{i_effect})            
            for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                for i_anatlabels = 1:length(anatreg{i_effect}.label_aggregated)
                    if contains(anatlabel_sourceelec{i_sub}{i_sourceelec}, ...
                        ...
                        anatreg{i_effect}.sourceelec_index{i_sub}(i_sourceelec,1) = i_anatlabels;
                        anatreg{i_effect}.sourceelec_label{i_sub}{i_sourceelec,1} = anatreg{i_effect}.label_aggregated{i_anatlabels};
                    end
                end
            ...
            for i_targetelec = 1:length(GCdata{i_sub}.ind_pairedelecs_to{i_effect})
                for i_anatlabels = 1:length(anatreg{i_effect}.label_aggregated)
                    if ~isempty(anatlabel_targetelec{i_sub}(:,sel_sourceelec))
                        if contains(anatlabel_targetelec{i_sub}{i_targetelec,sel_sourceelec}, ...
                                anatreg{i_effect}.label_unique{i_anatlabels})
                            anatreg{i_effect}.targetelec_index{i_sub}(i_targetelec,1) = ...
                                i_anatlabels;
                            anatreg{i_effect}.targetelec_label{i_sub}{i_targetelec,1} = ...
    ...
    %2.3 Pool elec pair-wise GC estimates across subjects per selected anatomical region
    ...
    for i_TD = 1:length(ToneDur_text)
        ...
        for i_anatlabels_source = 1:length(anatreg{i_effect}.label_aggregated)
            for i_anatlabels_target = 1:length(anatreg{i_effect}.label_aggregated)
                ...
                for i_sub = 1:length(subs)
                    if ~isempty(GCdata{i_sub}.temporalGC{i_effect})
                        %Filter: Select GC value only if corresponing source elec agrees with current anat reg
                        for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                            if any(anatreg{i_effect}.sourceelec_index{i_sub}(i_sourceelec) == ... 
                                    anatreg{i_effect}.labelindex_aggregated(i_anatlabels_source,:))

                                %Filter: Select GC value only if corresponing target elec agrees with current anat reg
                                for i_targetelec = 1:length(GCdata{i_sub}.ind_pairedelecs_to{i_effect})
                                    if any(anatreg{i_effect}.targetelec_index{i_sub}(i_targetelec) == ...
                                        ...
                                        disp(['Found GC value for source loc: ' anatreg{i_effect}.label_aggregated{i_anatlabels_source} ...
                                            ...
                                            GCdata{i_sub}.label_allelecs{GCdata{i_sub}.ind_pairedelecs_to{i_effect}(i_targetelec)} ')' ...
                                            ' for sub: ' subs{i_sub}]);
                                        
                                        %Pool GC across electrode pairs from same anatomical region
                                        temp_tempGC.source2target = ...
                                            [temp_tempGC.source2target; ...
                                            squeeze(GCdata{i_sub}.temporalGC{i_effect}{i_TD}.source2target(i_sourceelec, i_targetelec, :))'];
                                        temp_tempGC.target2source = ...
                                            [temp_tempGC.target2source; ...
                                            squeeze(GCdata{i_sub}.temporalGC{i_effect}{i_TD}.target2source(i_sourceelec, i_targetelec, :))'];
                                        
                                        temp_spectGC_repcount = temp_spectGC_repcount + 1;
                                        temp_spectGC.source2target(:,:,temp_spectGC_repcount) = ...
                                            squeeze(GCdata{i_sub}.spectralGC{i_effect}{i_TD}.source2target(i_sourceelec, i_targetelec, :, :));
                                        temp_spectGC.target2source(:,:,temp_spectGC_repcount) = ...
                                            squeeze(GCdata{i_sub}.spectralGC{i_effect}{i_TD}.target2source(i_sourceelec, i_targetelec, :, :));
                                    end
                                end
                            end
                        end
        ...
        for i_sourceelec = 1:length(anatreg{i_effect}.label_aggregated)
            for i_targetelec = 1:length(anatreg{i_effect}.label_aggregated)
                for i_toneepoch = 1:length(param.GC.tone_aggregation)
                    if any(~isnan(squeeze(Gavg_tempGC_peranatreg.source2target{i_effect}...
...
%3.1 Plot avg and SEM of GC estimates for each anatomical region as a
...
for i_TD = 1:length(ToneDur_text)
    ...
    sgtitle({['Time-domain pooled (n = ' num2str(length(subs)) ...
        ') pw GC estimates (mean +- SEM) per tone group per anat region'], ...
        [ToneDur_text{i_TD} 's TD; ' ...
        param.GC.InputDataType{1} ', ' param.GC.epochdur_ms_label]}, 'Interpreter', 'none')
    subplot_num = 0;
    
    %3.1.1 Subplot 1/1 Pred-Pred & Frontal-Parietal
    i_effect = 1;
    ...
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        nanmean(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(1),temp_currfilt(2),i_TD}), ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(1),temp_currfilt(2),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'b-o','markerfacecolor','b'},'transparent',1);
    hold on;
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        nanmean(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(2),temp_currfilt(1),i_TD}), ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(2),temp_currfilt(1),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'r-o','markerfacecolor','r'},'transparent',1);
    
    f_sub.XTick =  [1:length(param.GC.tone_aggregation)];
    f_sub.XTickLabel = param.GC.tone_aggregation;
    ...
    title(['Pred -> Pred; Frontal / Parietal'], ...
        'fontsize', 12);
    
    %3.1.2 Subplot 2/1 Pred-Pred & Frontal-Temporal
    subplot_num = 3;
    ...
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(1),temp_currfilt(3),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'b-o','markerfacecolor','b'},'transparent',1);
    hold on;
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        nanmean(Gavg_tempGC_peranatreg.source2target{i_effect}...
        ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(3),temp_currfilt(1),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'r-o','markerfacecolor','r'},'transparent',1);
    ...
    title(['Pred -> Pred; Frontal - Temporal'], ...
        'fontsize', 12);

    %3.1.3 Subplot 3/1 Pred-Pred & Parietal-Temporal
    subplot_num = 5;
    ...
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(2),temp_currfilt(3),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'b-o','markerfacecolor','b'},'transparent',1);
    hold on;
    shadedErrorBar(1:length(param.GC.tone_aggregation), ...
        nanmean(Gavg_tempGC_peranatreg.source2target{i_effect}...
        ...
        (nanstd(Gavg_tempGC_peranatreg.source2target{i_effect}...
        {temp_currfilt(3),temp_currfilt(2),i_TD})./sqrt(length(subs))), ...
        'lineprops',{'r-o','markerfacecolor','r'},'transparent',1);
    ...
    title(['Pred -> Pred; Parietal - Temporal'], ...
        'fontsize', 12); 
    
    
    %3.2.1 Subplot 1/1 PE-PE & Frontal-Parietal
    i_effect = 2;
    ...
    title(['PE -> PE; Frontal / Parietal'], ...
        'fontsize', 12);
    
    %3.2.2 Subplot 2/1 PE-PE & Frontal-Temporal
    subplot_num = 4;
    ...
    title(['PE -> PE; Frontal - Temporal'], ...
        'fontsize', 12);

    %3.2.3 Subplot 3/1 PE-PE & Parietal-Temporal
    subplot_num = 6;
    ...
    %3.3.1 Subplot 1/1 Pred-PE & Frontal-Parietal
    i_effect = 3;
    ...
    title(['Pred -> PE; Frontal / Parietal'], ...
        'fontsize', 12);
    
    %3.3.2 Subplot 2/1 Pred-PE & Frontal-Temporal
    subplot_num = 3;
    ...
    title(['Pred -> PE; Frontal - Temporal'], ...
        'fontsize', 12);

    %3.3.3 Subplot 3/1 Pred-PE & Parietal-Temporal
    subplot_num = 5;
    ...
    title(['Pred -> PE; Parietal - Temporal'], ...
        'fontsize', 12); 
    
    
    %3.4.1 Subplot 1/1 PE-Pred & Frontal-Parietal
    i_effect = 4;
    ...
    title(['PE -> Pred; Frontal / Parietal'], ...
        'fontsize', 12);
    
    %3.4.2 Subplot 2/1 PE-Pred & Frontal-Temporal
    subplot_num = 4;
    ...
    title(['PE -> Pred; Frontal - Temporal'], ...
        'fontsize', 12);

    %3.4.3 Subplot 3/1 PE-PE & Parietal-Temporal
    subplot_num = 6;
    ...
    title(['PE -> Pred; Parietal - Temporal'], ...
        'fontsize', 12);     
