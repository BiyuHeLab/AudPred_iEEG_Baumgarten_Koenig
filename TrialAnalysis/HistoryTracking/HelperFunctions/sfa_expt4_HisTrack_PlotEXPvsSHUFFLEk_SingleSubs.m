function sfa_expt4_HisTrack_PlotEXPvsSHUFFLEk_SingleSubs(sub, tonedur_text, baseline_correct, nReps, pval, ShuffleSets, saveplot)
...
%         dat.avg = dv; %all ECoG chan
        ...
        if saveplot
            if strcmp(ShuffleSets, 'appended')
                path_fig1 = [path_fig '2DLineplot/AppendedShuffleSets/'];
                filename     = [sub '_signK_appSHUFFLEsets_2DLine_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
            elseif strcmp(ShuffleSets, 'averaged')
                path_fig1 = [path_fig '2DLineplot/AvgShuffleSets/'];
                filename     = [sub '_signK_avgSHUFFLEsets_2DLine_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
            end
            mkdir([path_fig1]);
...
%         clims = [1,max(vals)]; %colormap limits
        ...
        if saveplot
            if strcmp(ShuffleSets, 'appended')
                path_fig2 = [path_fig '3DSurfplot/AppendedShuffleSets/'];
                filename     = [sub '_signK_appSHUFFLEsets_3DSurf_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
            elseif strcmp(ShuffleSets, 'averaged')            
                path_fig2 = [path_fig '3DSurfplot/AvgShuffleSets/'];
                filename     = [sub '_signK_avgSHUFFLEsets_3DSurf_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
           end
            mkdir([path_fig2]);
    ...
    if strcmp(sub,'NY723') %For subject 4 also plot RH
            ...
            if saveplot
                if strcmp(ShuffleSets, 'appended')
                    path_fig2 = [path_fig '3DSurfplot/AppendedShuffleSets/'];
                    filename     = [sub '_signK_appSHUFFLEsets_3DSurfRH_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
                elseif strcmp(ShuffleSets, 'averaged')            
                    path_fig2 = [path_fig '3DSurfplot/AvgShuffleSets/'];
                    filename     = [sub '_signK_avgSHUFFLEsets_3DSurfRH_' w1 '-' w2 'msTW_' tonedur_text 'msTD_p' num2str(pval) '.png'];
               end
                mkdir([path_fig2]);
                figfile      = [path_fig2 filename];                
