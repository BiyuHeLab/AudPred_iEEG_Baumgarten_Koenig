function sfa_expt4_HisTrack_CompSaveKval_GAMMA(sub, baseline_correct, train_odd, tonedur_text)
%TJB: Adapted version of tone_history_CV_SLIDE16.m
...
%1) Compute k-values for both train and test set (SLIDE16 = last 16 tones, sliding time window)
    %K-values are computed for al ECoG channels, selection of good vs. bad
    %channels is performed later
...
%2.4 Check if data is present for all sensors & timepoints in all trials
...
fsample              = data_ECoGfiltref_trials_train.fsample;
...
%% Note 03/28/19: 
%For NY688, we only have the recording with fsample = 512 Hz. For other we 
%have recording freq at 512 and 2048 Hz. 512Hz results in an non-integer 
%number of samples per tone (102.4 samples for a 200 ms
%tone and 204.8 samples for a 400 s tone). This causes difficulties in the
%later analysis, since we cannot cleanly track ECoG activity per tone.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%4.2 Compute across-time-point-averaged Gamma power for each sensor, time window, trial, and tone for the training set

for toneIndex = 1:33 %loop across tones
    for i_trial = 1:nTrials_train %loop across training trials

        %Manual version GammaAmplitude extimation with Hilbert transform
        filterOrder = 3; 
        
        lowPassFreq = 120; %first low-pass filter
        [lpb,lpa] = butter(filterOrder,(lowPassFreq*2)/fsample,'low');
        gammaAmp_tone_trial = filtfilt(lpb,lpa,data_ECoGfiltref_trials_train.trial{i_trial}(:, t_start_ind(toneIndex) : t_end_ind(toneIndex)));
        %output = channels*time points (for specific tone in specific trial)
        
        highPassFreq = 50; %then high-pass
        [lpb,lpa] = butter(filterOrder,(highPassFreq*2)/fsample,'high');
        gammaAmp_tone_trial = filtfilt(lpb,lpa,gammaAmp_tone_trial);
      
        gammaAmp_tone_trial = (abs(hilbert(gammaAmp_tone_trial))); %take absolute hilbert as amplitude envelope
        
        gammaAmp_tone_trial = log10(gammaAmp_tone_trial) - repmat(mean(log10(gammaAmp_tone_trial),2), [1 size(gammaAmp_tone_trial,2)]);
        %Correction: log of gamma amplitude per sensor/timepoint corrected
        %by average across time points (log of gamma amplitude per sensor
        %across time points)
        
        %get mean Gamma amp for all windows for current tone
        for i_win = 1:size(windows,1)
            ind_start = windows(i_win, 1);
            ind_end   = windows(i_win, 2);

            gamma_win_train{i_win}{toneIndex}(:, i_trial) = mean(gammaAmp_tone_trial(:, ind_start:ind_end),2);
...
for toneIndex = 1:33 %loop across tones
    for i_trial = 1:nTrials_test %loop across testing trials

        %Manual version GammaAmplitude extimation with Hilbert transform
        filterOrder = 3; 
        
        lowPassFreq = 120; %first low-pass filter and resitriction to specific tone within trial
        [lpb,lpa] = butter(filterOrder,(lowPassFreq*2)/fsample,'low');
        gammaAmp_tone_trial = filtfilt(lpb,lpa,data_ECoGfiltref_trials_test.trial{i_trial}(:, t_start_ind(toneIndex) : t_end_ind(toneIndex)));
        %output = channels*time points (for specific tone in specific trial)
        ...
        for i_win = 1:size(windows,1)
            ...
            gamma_win_test{i_win}{toneIndex}(:, i_trial) = mean(gammaAmp_tone_trial(:, ind_start:ind_end),2);
            %TJB: output = Time-point-averaged MEG activity for all channels, for current trial, organized per time window, per tone
        end
    end
end

% %% Test: plot filtered data 
%     % to check for suspicious subjects/time windows 
%     test_struct = data_ECoGfiltref_trials_test;
%         %perform Gamma amplitude envelope for entire trial
%          for i_trial = 1:nTrials_test %loop across testing trials
%                 %Manual version GammaAmplitude extimation with Hilbert transform
%                 filterOrder = 3;         
%                 lowPassFreq = 120; %first low-pass filter and resitriction to specific tone within trial
%                 [lpb,lpa] = butter(filterOrder,(lowPassFreq*2)/fsample,'low');
%                 gammaAmp_tone_trial = filtfilt(lpb,lpa,data_ECoGfiltref_trials_test.trial{i_trial});
%                 %output = channels*time points (for specific tone in specific trial)
%                 highPassFreq = 50; %then high-pass
%                 [lpb,lpa] = butter(filterOrder,(highPassFreq*2)/fsample,'high');
%                 gammaAmp_tone_trial = filtfilt(lpb,lpa,gammaAmp_tone_trial);
...
%                 gammaAmp_tone_trial = (abs(hilbert(gammaAmp_tone_trial))); %take absolute hilbert as amplitude envelope
% 
%                 test_struct.trial{i_trial} = log10(gammaAmp_tone_trial) - repmat(mean(log10(gammaAmp_tone_trial),2), [1 size(gammaAmp_tone_trial,2)]);
%          end
% 
%         %Plot in time domain
%         cfg = [];
%         cfg.channel = data_ECoGfiltref_trials.label(test_struct.cfg.info_elec.selected.index4EDF); %selected channels
%         cfg.ylim = 'maxabs';
%         cfg.viewmode = 'vertical';
%         cfg = ft_databrowser(cfg,test_struct);
% 
%         %Plot in freq domain
%         figure
%         set(gcf,'units','normalized','outerposition',[0 0 1 1]) %fullscreen%for i_chan = 100 %length(data_ECoGraw_blocks.label)
%         for i_trial = 1:length(test.sampleinfo)      
%             subplot(ceil(length(test_struct.sampleinfo)/5),ceil(length(test_struct.sampleinfo)/3),i_trial);    
%             pwelch(test_struct.trial{i_trial},[],[],[],512); %filtered data
    ...
    for i_win = 1:size(windows,1)
        for i_sensor = 1:nSensors            
            ...
            for toneIndex = 16:32 %17 entries
                dv = [dv; gamma_win_train{i_win}{toneIndex}(i_sensor, :)']; %Column vector with MEG activity for time window at current tone
...
for i_k = 1:length(ks)
    ...
    for i_win = 1:size(windows,1)
        for i_sensor = 1:nSensors
            ...
            dv = [];
            for toneIndex = 16:32
                dv = [dv; gamma_win_test{i_win}{toneIndex}(i_sensor, :)'];
            ...
            ss.R2_test   = R2_test;
            ss.sig2_test = sig2_test; %%Note: This is the critical k-value by which we later define the k-prime value
            ss.AIC_test  = n*log(sig2_test) + 2*K;
            ss.BIC_test  = n*log(sig2_test) + K*log(n);
            ...
            K    = length(ss2.tstat.beta) + 1;
            
            ss.R2_test_regr   = ss2.rsquare; %r-square for test set based on regression for test set
            ss.sig2_test_regr = sig2_test_regr;
            ss.AIC_test_regr  = n*log(sig2_test_regr) + 2*K;
            ss.BIC_test_regr  = n*log(sig2_test_regr) + K*log(n);
            
                       
            stats_linear{i_win, i_k, i_sensor} = ss; %Add test stats to common stats output var   
...
% %% Test: Read out sig_k vals for checking
% n_k = 16;
% k_list = 1:n_k;
% for i_win = 1:size(windows,1)
%     for i_sensor = 1:nSensors
...
%         sig2_test{i_win}(i_sensor, :) = sig2s; %sum of squared residuals divided by length of model order
...
%         sig2_test_k{i_win}(i_sensor) = k_list(min_s_ind); %k-prime-value (i.e., preferred model order/Number of tones back best explaining MEG data)
%         sig2_test_k_sig{i_win}(i_sensor) = min_s; %minimum squared residuals divided by length of model order of K-value (i.e., preferred model order/Number of tones back)
%            
%     end
% end
...
%8.2 save output var
savefile = [path_save sub '_HisTrackGAMMAEnvSLIDE16_regstatsEXP_' BC_text '_' tonedur_title 'msToneDur_' oddity_text '.mat'];
save(savefile, 'stats_linear');           
