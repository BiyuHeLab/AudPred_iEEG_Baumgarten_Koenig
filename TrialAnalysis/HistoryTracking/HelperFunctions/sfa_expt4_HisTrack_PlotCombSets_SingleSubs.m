function sfa_expt4_HisTrack_PlotCombSets_SingleSubs(sub, tonedur_text, baseline_correct, saveplot)
...
%% 1) Load combined k-value data
 load([path_load sub '_HisTrackSLIDE16_regstatsEXP_' BC_text '_' tonedur_title 'msToneDur_kModelComp_CombSets.mat']);
...
%         dat.avg = dv; %all ECoG chan
        dat.avg = dv(data_ECoGfiltref_trials.cfg.info_elec.selected.index4EDF); %seleced ECoG chan
        ...
        if saveplot
            ...
            mkdir([path_fig1]);
            filename     = [sub '_KprimeperElec_2DLine_' w1 '-' w2 'msTimeWin_' tonedur_text 'msToneDur.png'];
...
%         clims = [1,max(vals)]; %colormap limits
        ...
        if saveplot
            ...
            mkdir([path_fig2]);
            filename     = [sub '_KprimeperElec_3DSurf_' w1 '-' w2 'msTimeWin_' tonedur_text 'msToneDur.png'];
        ...
        if strcmp(sub,'NY723') %For subject 4 also plot RH
            ...
            if saveplot
                path_fig2 = [path_fig '3DSurfplot/'];
                mkdir([path_fig2]);
                filename     = [sub '_KprimeperElec_3DSurfRH_' w1 '-' w2 'msTimeWin_' tonedur_text 'msToneDur.png'];
                figfile      = [path_fig2 filename];                
