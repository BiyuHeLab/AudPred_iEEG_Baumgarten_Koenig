...
path_fig  = '/data/gogodisk2/brian/analysis/sfa_expt2_v2/stimulus_tracking/stim_corr/figures_avg/cluster_within_x_sigma/';
path_data_cluster = '/data/gogodisk2/brian/analysis/sfa_expt2_v2/stimulus_tracking/stim_corr/stim_corr_results/cluster_within_x_sigma/';
...
if load_preexisting_data
    ...
    for i_sub = 1:length(subs)
        ...
        for i_sensor = 1:nSensors
            ...
            sigma_s  = Sigma(i_sensor, :);
            
            ind = 0;
            for i_within = 0:1
                for i_sigma = 1:2
                    ind = ind + 1;
                    dv{i_sensor}(i_sub, ind) = mean(zvals_s(within_s == i_within & sigma_s == i_sigma));
                    f_sub(i_sub, ind) = i_sub;
                    f_within(i_sub, ind) = i_within;
                    f_sigma(i_sub, ind) = i_sigma;
                end
            end
        end
...
%     analysis = [analysis, 'test_stat = stats{2, 5};  '];
    ...
    t0 = GetSecs;
    [clusters_orig, xx, shuffleMaxStat] = cluster_test_rm_anova2(dv, f_sub, f_within, f_sigma, f_names, p_thresh, analysis, nReps);
    tf = GetSecs - t0;
...
title({['Effect of Within x Sigma on stimulus correlation (n = ' num2str(nSubs) ')'], ...
