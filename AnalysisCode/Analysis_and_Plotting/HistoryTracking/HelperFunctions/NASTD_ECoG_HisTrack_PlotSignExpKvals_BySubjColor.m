function NASTD_ECoG_HisTrack_PlotSignExpKvals_BySubjColor...
...
if FDR_correct == 1
    ...
    data_selElec_allSubs.index_signkprime_avgTW_FDR = pval_FDRcorrected < pval_plotting;    
    sumSignELec             = sum(data_selElec_allSubs.index_signkprime_avgTW);
    FDR_label               = 'FDRcorrected';
    index_signelecs = find(logical(...
        data_selElec_allSubs.index_signkprime_avgTW_FDR));
else
    data_selElec_allSubs.index_signkprime_avgTW_FDR = ...
        false(length(data_selElec_allSubs.index_signkprime_avgTW),1);
    sumSignELec             = sum(data_selElec_allSubs.index_signkprime_avgTW);
    FDR_label               = 'uncorrected';
    index_signelecs = find(logical(...
        data_selElec_allSubs.index_signkprime_avgTW));
...
%Optional: Plot electrodes projected on left hemisphere
edges = 0.5:1:8.5;

for i = 1:length(edges)-1 % for different value ranges of k' plot by subject color
    close all;
    figure('visible','off'); %ensures figure doesn't pop up during plotting
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen

    %Create electrode labels for plotting
    for i_elec = 1:length(vals)%No electrode labels
        labels_plotting_empty{i_elec} = '';
    end
    dat.textcolor_rgb   = [1 0 0];
    SubplotPosition = [0 0 0 0];
    %Project all electrodes on one hemisphere,
    coords_allsub = coords;
    coords_allsub(:,1) = abs(coords(:,1)) * -1;
    
    lower_edge = edges(i);
    upper_edge = edges(i+1);
    
    % Define mask: values within interval & significant electrodes
    mask = (vals > lower_edge & vals <= upper_edge) & (sign_index ~= 0);

    % Check if at least one electrode in mask
    if sum(mask) < 2
        fprintf('No electrodes in range (%.1f, %.1f], skipping plot.\n', lower_edge, upper_edge);
        continue;
    end

    vals_subjID = data_selElec_allSubs.sub;
    vals_subjID(~mask) = NaN;

    % Apply mask to sign_index
    sign_index_new = sign_index;
    sign_index_new(~mask) = 0;  % Keep only significant electrodes in range

    NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH...
        (coords_allsub, labels_plotting_empty, vals_subjID, sign_index_new,...
        chanSize, clims, cmap, dat.textcolor_rgb, ...
        [1 1], 1, SubplotPosition, [], []);

    ColorbarPosition    = [0.875 0.05 0.035 0.95]; %Left of surface
    h = colorbar;
    ...
    title(sprintf('k'' values in range (%.1f, %.1f]', lower_edge, upper_edge));

    if save_poststepFigs == 1
        filename     = ['3DSurf_SignExpKprime_Allsub_' input_DataLabel ...
            '_' tonedur_title '_' Label_TW 'Avgall_p' ...
            num2str(pval_plotting) FDR_label ...
            '_' param.ElecSelect 'elec_k' num2str(lower_edge + 0.5) '.png'];
