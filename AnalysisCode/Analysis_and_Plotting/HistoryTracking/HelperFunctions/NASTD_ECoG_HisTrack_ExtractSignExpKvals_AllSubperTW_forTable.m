function ExpKprime_data_perTW = NASTD_ECoG_HisTrack_ExtractSignExpKvals_AllSubperTW_forTable...
...
%     clims       = [0 max([dv_absmax, 1.1])]; %free scaling
% clims       = [0 15]; %fixed scaling
clims       = [min(vals)+1 max(vals)-1]; %fixed scaling
cmap        = 'jet';
chanSize    = (vals./vals)*1; %electrode size (arbitrary)
...
% C.Position(2) = C.Position(2)*0.7; %lower Colorbar
    ...
    chanSize(data_selElec_allSubs.index_signkprime_avgTW_FDR) = 2.5;    

    NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH...
            (coords_allsub, labels_plotting_empty, vals, sign_index,...
