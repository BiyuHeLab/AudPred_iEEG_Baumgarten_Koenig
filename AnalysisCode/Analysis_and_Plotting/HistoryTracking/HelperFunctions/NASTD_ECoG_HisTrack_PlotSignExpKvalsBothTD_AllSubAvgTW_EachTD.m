function NASTD_ECoG_HisTrack_PlotSignExpKvalsBothTD_AllSubAvgTW_EachTD...
...
%Select only electrodes significant for each or both TDs
sig_TD1_only = data_selElec_allSubs.filt_signElecs_avgWin{1} & ...
               ~data_selElec_allSubs.filt_signElecs_avgWin{2};

sig_TD2_only = data_selElec_allSubs.filt_signElecs_avgWin{2} & ...
               ~data_selElec_allSubs.filt_signElecs_avgWin{1};

sig_both     = data_selElec_allSubs.filt_signElecs_avgWin{1} & ...
               data_selElec_allSubs.filt_signElecs_avgWin{2};
...
% chanSize = NaN(size(vals));
% 
% % Assign chanSize for electrodes from each TD
% chanSize(sig_TD1_only) = 1;
% chanSize(sig_TD2_only) = 3;
 chanSize(sig_both) = 3;
...
DimSubplot = [1, 2]; %Surface left, 2D plot right
num_subplot = 1;
%Project all electrodes on one hemisphere,
coords_projectedL = coords;
coords_projectedL(:,1) = abs(coords(:,1)) * -1;

% Plot only for electrodes sig for 1 TD
vals(sig_TD2_only) = NaN;
sign_index(sig_TD2_only) = 0;
...
hold on;
scatter(...
    data_selElec_allSubs.kprime_selelecs_perTD_avgWin{1}(sig_both), ...
    data_selElec_allSubs.kprime_selelecs_perTD_avgWin{2}(sig_both), ...
    50, ... % slightly larger for visibility
    'o', 'MarkerFaceColor','none', 'MarkerEdgeColor','k', 'LineWidth',1.5);
hold on;
