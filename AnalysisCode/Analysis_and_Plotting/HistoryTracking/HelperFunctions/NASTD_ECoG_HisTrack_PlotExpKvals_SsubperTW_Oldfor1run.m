...
load([path_inputdata sub '_' input_DataLabel '_' tonedur_title ...
    '_HisTrack_ExpKprimeCombFolds_unbalanced.mat']); %filename: ExpKprime_data, labels_loadedData
...
%2.2 Determine selected electrode EDF-index to read out elec coords
SelElec_MNIindex   = [];
for i_selelec = 1:length(labels_loadedData)
    SelElec_MNIindex = [SelElec_MNIindex, ...
        find(strcmp(DataClean_AllTrials.elec.label, ...
        labels_loadedData{i_selelec}))];
end
...
for i_win = 1:size(windows,1)
    dv_win_absmax = max(ExpKprime_data.avgFolds.Kprime_Avg{i_win});
...
for i_win = 1:size(windows,1)
    ...
    win_title = {['TW = ' w1 ' - ' w2 ' ms '],...
        ...
        num2str(mean(ExpKprime_data.avgFolds.Kprime_Avg{i_win})) ...
        ' (' num2str(std(ExpKprime_data.avgFolds.Kprime_Avg{i_win})) ')']};
    ...
    dv          = ExpKprime_data.avgFolds.Kprime_Avg{i_win};
    dat.avg     = dv; %
    
    %3.2.1) Plot electrodes as spheres on MNI brain with each sphere color-coded
    %depending on Kprime-value
    coords      = DataClean_AllTrials.elec.chanpos(SelElec_MNIindex,:); %MNI coordinates for selected electrodes
    ...
    if plot_SubplotperTW == 0 %one separate figure per TW
        ...
        if save_poststepFigs == 1
            filename     = ['3DSurf_ExpKprime_' sub '_' input_DataLabel ...
                '_' tonedur_title '_' Label_TW num2str(i_win) '.png'];
...
if plot_SubplotperTW == 1 %one subplot per TW
    ...
    if save_poststepFigs == 1
        filename     = ['3DSurf_ExpKprime_' sub '_' input_DataLabel '_' ...
            tonedur_title '_' Label_TW 'all.png'];
