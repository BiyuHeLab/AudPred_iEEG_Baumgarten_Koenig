...
for i_tonedur = 1:length(tonedur_text) %tone duration condition
    ...
    for i_win = 1:size(windows,1)
        ...
        dat.sign_elecs = logical(data_AllElec.sign_elecs{i_tonedur}{i_win});
            ...
            dat.sign_elecs(find(strcmp(dat.label,'G63'))) = 1; %A1
            dat.sign_elecs(find(strcmp(dat.label,'G46'))) = 1; %STG
            dat.sign_elecs(find(strcmp(dat.label,'IF1'))) = 1; %IFG
        
            dat.avg(find(strcmp(dat.label,'G63'))) = -3; %A1 - blue
            dat.avg(find(strcmp(dat.label,'G46'))) = 3; %STG - red
            dat.avg(find(strcmp(dat.label,'IF1'))) = 1; %IFG - yellow
...
%         Figtitle = {['All subs (n = ' num2str(length(sub_list)) ') - ' Fig_title],['ToneDur:' tonedur_text{i_tonedur} 'ms - ERF ' win_title ' - p' num2str(pval)]};
        Figtitle = {'NY688 electrode selection for p34 ERP analysis - A1 (blue), STG (red), IFG (yellow)'};
        title(Figtitle)
