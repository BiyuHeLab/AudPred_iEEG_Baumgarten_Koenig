function sp_handle_surf = NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH...
...
chanSize_nonsignElec = ones(length(chanSize));%chanSize/3;
...
if any(sign_index) %if there are any sign. channels
    ...
    sign_chansize = chanSize(find(sign_index));
    ...
    for cn = 1:numChans
        x = (sign_chansize(cn)*xs) + t.vertices(nearestVertex(cn),1);
        ...
        y = (sign_chansize(cn)*ys) + t.vertices(nearestVertex(cn),2);
        z = (sign_chansize(cn)*zs) + t.vertices(nearestVertex(cn),3);
