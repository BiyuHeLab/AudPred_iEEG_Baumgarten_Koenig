function c = NASTD_ECoG_Plot_SubplotElecsSurf...
...
% subplot(DimSubplot(1),DimSubplot(2),CounterSubplot); %no sublot-location size change
sp_handle1 = subplot(DimSubplot(1),DimSubplot(2),CounterSubplot); 
pos1 = get(sp_handle1,'Position');
% newpos1 = pos1 + [0 0 0 DimSubplot(2)*0.1]; %Enlarge subplot
...
%     h.Position(4) = h.Position(4)*0.6; %makes colorbar shorter
    ylabel(h, 'Exp Kprime (no threshold - all elecs)')
    yticklabels('auto');
    % set(gca,'FontSize',16);
else 
    colorbar off
