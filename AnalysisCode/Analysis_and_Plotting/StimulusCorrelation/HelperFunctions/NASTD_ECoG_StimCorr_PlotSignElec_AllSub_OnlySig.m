function NASTD_ECoG_StimCorr_PlotSignElec_AllSub_OnlySig ...
    (subs, ...
...
for i_sub = 1:length(subs)
    ...
    Param2plot.per_sub.corrcoeff_identical{i_sub}   = nan(nSensors, 1);
    Param2plot.per_sub.corrcoeff_similar{i_sub}     = nan(nSensors, 1);
    Param2plot.per_sub.corr_ttest_t{i_sub}          = nan(nSensors, 1);
    Param2plot.per_sub.corr_ttest_p{i_sub}          = nan(nSensors, 1);
    Param2plot.per_sub.anova_F{i_sub}               = nan(nSensors, 1);
    Param2plot.per_sub.anova_p{i_sub}               = nan(nSensors, 1);
            
    Param2plot.per_sub.corrcoeff_identical{i_sub}(:,1) = corrcoeff_identical;
    Param2plot.per_sub.corrcoeff_similar{i_sub}(:,1)   = corrcoeff_similar;
    Param2plot.per_sub.corr_ttest_t{i_sub}(:,1)        = corr_ttest.t;
    Param2plot.per_sub.corr_ttest_p{i_sub}(:,1)        = corr_ttest.p;
    Param2plot.per_sub.anova_F{i_sub}(:,1)             = anova_F;
    Param2plot.per_sub.anova_p{i_sub}(:,1)             = anova_p;
    
    %Array to differentiate subject entries
    Param2plot.all_subs.sub_index = ...
        [sub_index; ones(length(coords_sub{i_sub}),1)*i_sub];
    
    %data aggregation over subjects
    if i_sub == 1
        Param2plot.all_subs.corrcoeff_identical     = ...
            Param2plot.per_sub.corrcoeff_identical{i_sub};
        Param2plot.all_subs.corrcoeff_similar       = ...
            Param2plot.per_sub.corrcoeff_similar{i_sub};
        Param2plot.all_subs.corr_ttest_t            = ...
            Param2plot.per_sub.corr_ttest_t{i_sub};
        Param2plot.all_subs.corr_ttest_p            = ...
            Param2plot.per_sub.corr_ttest_p{i_sub};
        Param2plot.all_subs.anova_F                 = ...
            Param2plot.per_sub.anova_F{i_sub};
        Param2plot.all_subs.anova_p                 = ...
            Param2plot.per_sub.anova_p{i_sub};
    else
        Param2plot.all_subs.corrcoeff_identical     = ...
            [Param2plot.all_subs.corrcoeff_identical; ...
            Param2plot.per_sub.corrcoeff_identical{i_sub}];
...
sgtitle({...
    ...
    FuncInput_ToneDur_text 's TD - (RH elecs projected on LH)']}, ...
    'Interpreter','none')
...
plot_struct.chanSize        = ...
    ones(1,length(plot_struct.sign_elecs))*SizeFactor/2; %electrode size (arbitrary)
...
plot_struct.avg(~SignElecs.array) = nan;
...
if save_poststepFigs == 1
    filename     = ['StimCorr_Surf1HSignElec_n' num2str(length(subs)) '_' ...
        FuncInput_DataType '_' FuncInput_ToneDur_text 'sTD_OnlySig.png'];
