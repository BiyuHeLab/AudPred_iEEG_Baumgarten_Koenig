...
% My new code which doesn't yet work for plotting (but does count as an
% appraoch to finding the sig electrodes across both tone durs)
...
%     temp_indexsignelec(i_elec) = min(Param2plot.all_subs.corr_ttest_p(index_of_bothToneDurs));
% end
% 
% SignElecs.array         = temp_indexsignelec < param.pval_plotting;

% Old code from Thomas
...
plot_struct.sign_elecs      = logical(zeros(length(Param2plot.all_subs.corr_ttest_t),1));
plot_struct.sign_elecs(SignElecs.index) = 1;
...
%% 4. LK Edit June 9, 2025: For each input data type, check which electrodes are significant, and color code by subject ID. 
...
for tval_index = 1:3
    ...
    plot_struct.avg = subjID_sigelecs(SignElecs.index(data_to_plot));
    ...
    title({['Identical vs. similar sequences (p < ' num2str(param.pval_plotting) ' uncorr)'], ...
    ['# sign. elecs / all elecs: ' ...
    num2str(SignElecs.num_elecs) ' / ' num2str(sum(data_to_plot))]}, ...
    'FontSize',14)
end
