...
%     %3.3 Compute Frequency-band specific amplitude envelope and deal with NaNs
    ...
    for i_TD = 1:length(ToneDur_text)
        for i_freqbands = 4:length(FrequencyBand_Labels)            
...
%% 3. Comute stimulus correlation analyses
    %3.0 Parameters:
    InputDataType = {'HighGamma_LogAmp'};
...
% Plot correlation electrodes only for significant electrodes
for i_inputData = 1:length(InputDataType)
    for i_TD = 1:length(ToneDur_text)
        
        NASTD_ECoG_StimCorr_PlotSignElec_AllSub_OnlySig...
            (subs, ...
            InputDataType{i_inputData}, ToneDur_text{i_TD}, ...
            param,...
            save_poststepFigs, paths_NASTD_ECoG)
...
%     NASTD_ECoG_StimCorr_PlotSignElec_AllSubTD_lk_OnlySig...
%         (subs, ...
%         InputDataType{i_inputData}, ToneDur_text, ...
%         param,...
%         save_poststepFigs, paths_NASTD_ECoG)
    
      NASTD_ECoG_StimCorr_PlotSignElec_AllSubTD_lk_plotting...
        (subs, ...
        InputDataType{i_inputData}, ToneDur_text, ...
        param,...
        save_poststepFigs, paths_NASTD_ECoG)
...
%     save_poststepFigs, paths_NASTD_ECoG)
    ...
    NASTD_ECoG_StimCorr_PhaseDissim_new...
    (subs, ...
    InputDataType{i_inputData}, ToneDur_text, ...
    param,...
    save_poststepFigs, paths_NASTD_ECoG)
