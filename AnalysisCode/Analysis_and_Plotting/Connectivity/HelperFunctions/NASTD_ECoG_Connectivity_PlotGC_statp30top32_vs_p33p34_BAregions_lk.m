function NASTD_ECoG_Connectivity_PlotGC_statp30to32_vs_p33p34_BAregions_lk ...
    (subs, ...
    ToneDur_text, ...
...
% Define the field names of the structure
fields = {'source2target', 'target2source', 'subject_index'};

for fieldIdx = 1:numel(fields)
    currentField = fields{fieldIdx};  % Get the current field name
    
    % Check if the current field is indeed a 1x4 cell array
    if iscell(GCscores.(currentField)) && numel(GCscores.(currentField)) == 4
        % Loop through each of the 1x4 cells
        for arrayIdx = 1:4
            % Access the 10x10x2 cell array at this index
            currentArray = GCscores.(currentField){arrayIdx};
            
            % Ensure currentArray is indeed a 10x10x2 cell array
            if iscell(currentArray) && all(size(currentArray) == [10, 10, 2])
                % Iterate over the 10x10x2 cells within currentArray
                for i = 1:size(currentArray, 1)
                    for j = 1:size(currentArray, 2)
                        for k = 1:size(currentArray, 3)
                            cellContent = currentArray{i, j, k};
                            
                            % Ensure cell is not empty
                            if ~isempty(cellContent) && size(cellContent, 2) >= 5
                                % Average columns 1 to 3 and keep columns 4 and 5
                                col1to3_mean = mean(cellContent(:, 1:3), 2);
                                col4to5 = cellContent(:, 4:5);
                                
                                % Combine averaged and remaining columns into a new matrix
                                currentArray{i, j, k} = [col1to3_mean, col4to5];
                            end
                        end
                    end
                end
                
                % Store the modified 10x10x2 cell array back in the struct
                GCscores.(currentField){arrayIdx} = currentArray;
            else
                warning('Unexpected structure for %s at index %d', currentField, arrayIdx);
            end
        end
    else
        warning('%s does not have the expected 1x4 cell array structure.', currentField);
    end
end
