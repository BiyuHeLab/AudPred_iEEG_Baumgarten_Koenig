function NASTD_ECoG_Connectivity_PlotGC_statp1top31 ...
    (subs, ...
    ToneDur_text, ...
...
for i_TD = 1:length(ToneDur_text)
    ...
    %3.1.1 Subplot 1/1 Pred-Pred & Reg1 - Reg2
        ...
        %Fit lin. trend for each electrode and compare slopes vs. 0 with single sample t-test
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(1),temp_currfilt(2),i_TD},1)
            coeff_linfit_12(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(1),temp_currfilt(2),i_TD}(i_elec,:)), 1);
        end
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(2),temp_currfilt(1),i_TD},1)        
            coeff_linfit_21(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(2),temp_currfilt(1),i_TD}(i_elec,:)), ...
                1); 
            %Note: polyfit with polynomial order 1 returns the slope as 1st
            %parameter and the intercept as 2nd parameter
        end
        %t-test slope across electrodes vs. 0
        [~,pval_12,~,tstats_12] = ttest(coeff_linfit_12(:,1),0,'tail','both');
        [~,pval_21,~,tstats_21] = ttest(coeff_linfit_21(:,1),0,'tail','both');
        
        hold on;
        text(1.3, f_sub1.YLim(2)*0.75, ...
            {'Slopes LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_12(:,1)),6))) '; p = ' num2str(round(pval_12,3))]}, ...
            'Color',[0 0 1])
        text(1.3, f_sub1.YLim(2)*0.25, ...
            {'Slope LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_21(:,1)),6))) '; p = ' num2str(round(pval_21,3))]}, ...
            'Color',[1 0 0])
    ...
    %3.1.2 Subplot 1/1 Pred-Pred & Reg1 - Reg3
        ...
        %Fit lin. trend for each electrode and compare slopes vs. 0 with single sample t-test
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(1),temp_currfilt(3),i_TD},1)
            coeff_linfit_13(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(1),temp_currfilt(3),i_TD}(i_elec,:)), 1);
        end
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(3),temp_currfilt(1),i_TD},1)
            coeff_linfit_31(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(3),temp_currfilt(1),i_TD}(i_elec,:)), 1);            
        end
        [~,pval_13,~,tstats_13] = ttest(coeff_linfit_13(:,1),0,'tail','both');
        [~,pval_31,~,tstats_31] = ttest(coeff_linfit_31(:,1),0,'tail','both');
        
        hold on;
        text(1.3, f_sub1.YLim(2)*0.75, ...
            {'Slopes LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_13(:,1)),6))) '; p = ' num2str(round(pval_13,3))]}, ...
            'Color',[0 0 1])
        text(1.3, f_sub1.YLim(2)*0.25, ...
            {'Slope LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_31(:,1)),6))) '; p = ' num2str(round(pval_31,3))]}, ...
            'Color',[1 0 0])
    ...
    %3.1.3 Subplot 1/1 Pred-Pred & Reg2 - Reg3
        ...
        %Fit lin. trend for each electrode and compare slopes vs. 0 with single sample t-test
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(2),temp_currfilt(3),i_TD},1)
            coeff_linfit_23(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(2),temp_currfilt(3),i_TD}(i_elec,:)), 1);
        end
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(3),temp_currfilt(2),i_TD},1)
            coeff_linfit_32(i_elec,:) = ...
                polyfit...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(3),temp_currfilt(2),i_TD}(i_elec,:)), 1);            
        end
        [~,pval_23,~,tstats_23] = ttest(coeff_linfit_23(:,1),0,'tail','both');
        [~,pval_32,~,tstats_32] = ttest(coeff_linfit_32(:,1),0,'tail','both');
        
        hold on;
        text(1.3, f_sub1.YLim(2)*0.75, ...
            {'Slopes LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_23(:,1)),6))) '; p = ' num2str(round(pval_23,3))]}, ...
            'Color',[0 0 1])
        text(1.3, f_sub1.YLim(2)*0.25, ...
            {'Slope LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit_32(:,1)),6))) '; p = ' num2str(round(pval_32,3))]}, ...
            'Color',[1 0 0]) 
    ...
    %3.2.1 Subplot 2/1 PE-PE & Reg1 - Reg2    
        ...
        for i_elec = 1:size(Gavg_tempGC_peranatreg.source2target{i_effect}{temp_currfilt(2),temp_currfilt(1),i_TD},1)        
            coeff_linfit_21(i_elec,:) = ...
                ...
                (1:length(param.GC.tone_aggregation), ...
                (Gavg_tempGC_peranatreg.source2target{i_effect}...
                {temp_currfilt(2),temp_currfilt(1),i_TD}(i_elec,:)), 1);            
        end
        [~,pval_12,~,tstats_12] = ttest(coeff_linfit_12(:,1),0,'tail','both');
        [~,pval_21,~,tstats_21] = ttest(coeff_linfit_21(:,1),0,'tail','both');
