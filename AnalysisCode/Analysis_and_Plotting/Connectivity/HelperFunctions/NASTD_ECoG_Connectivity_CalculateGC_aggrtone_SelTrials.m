...
% 1.1.B) Select only trials of interest
DataSubset = DataClean_CleanTrials;
trialIDs_data = DataSubset.trialinfo(:,1);
% Identify corresponding trials in data for trialIDs input into
% function
[~,match_idx] = ismember(trialIDs, trialIDs_data); % Finds corresponding trials in the data
idx_seltrials_in_data = match_idx(match_idx > 0);
    
% Select only the trials for the given trial IDs
DataSubset.trial = DataClean_CleanTrials.trial(idx_seltrials_in_data);
DataSubset.time = DataClean_CleanTrials.time(idx_seltrials_in_data);
DataSubset.trialinfo = DataClean_CleanTrials.trialinfo(idx_seltrials_in_data, :);
DataSubset.sampleinfo = DataClean_CleanTrials.sampleinfo(idx_seltrials_in_data, :);

% SAve into DataClean_CleanTrials again
DataClean_CleanTrials = DataSubset;
...
for i_TD = 1:length(ToneDur_text)
    DataClean_CleanTrialsElecs_perTD{i_TD} = ...
        NASTD_ECoG_Preproc_SelTrialsperTD_SelTrials...
        (ToneDur_text{i_TD}, DataClean_CleanTrialsElecs, idx_seltrials_in_data);
...
%% TODO: For NY798, remove trials with >30% missing data/NaN samples
for i_TD = 1:length(ToneDur_text)
    DataClean_CleanTrialsElecs_perTD{i_TD} = ...
...
for i_effect = 1:length(param.GC.ElecPairEffect)
    ...
    if SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
        ...
        for i_TD = 1:length(ToneDur_text)
            ...
            trialData{i_TD} = trialData{i_TD}(ind_pairedelecs_both, :, :);
            %Note> Source and target elecs are vertically concatinated here
            %(i.e., source and target elecs are in the same file)
            ...
            validA{i_effect}{i_TD} = ... %Indicates if VAR coefficients matrix is valid
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2));            
            validF{i_effect}{i_TD} = ...%Indicates if GC computation is valid
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2)); 
            
            temporalGC{i_effect}{i_TD}.source2target = ...%pairwise-conditional time-domain multivariate Granger causalities, source->target
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2)); 
            temporalGC_pval{i_effect}{i_TD}.source2target = ...%pairwise-conditional frequency-domain/ spectral multivariate Granger causalites, source->target
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2)); 
            temporalGC{i_effect}{i_TD}.target2source = ...%pairwise-conditional time-domain multivariate Granger causalities, target->source
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2)); 
            temporalGC_pval{i_effect}{i_TD}.target2source = ...%pairwise-conditional frequency-domain/ spectral multivariate Granger causalites, target->source
                nan(size(ind_pairedelecs_from{i_effect},1), ...
                size(ind_pairedelecs_to{i_effect},1), ...
                size(param.GC.tone_aggregation,2)); 
            ...
            if param.GC.downsample > 0
                frequencyGC{i_effect}{i_TD}.source2target = ...
                    nan(size(ind_pairedelecs_from{i_effect},1), ...
                    size(ind_pairedelecs_to{i_effect},1), ...
                    (param.GC.fs/param.GC.downsample)+1, ...
                    size(param.GC.tone_aggregation,2));
                frequencyGC{i_effect}{i_TD}.target2source = ...
                    nan(size(ind_pairedelecs_from{i_effect},1), ...
                    size(ind_pairedelecs_to{i_effect},1), ...
                    (param.GC.fs/param.GC.downsample)+1, ...
                    size(param.GC.tone_aggregation,2));
            ...
            %dim: source elec * target elec * frequencies * tone aggregation
        end
                
        %% 4) Check input data for stationarity
        %Optional: check with reverse arrangement test if input data is stationary
%         for i_TD = 1:length(ToneDur_text)
%             h = waitbar(0, 'Testing stationarity across elecs, tones, and trials...');
%             
...
%                         %                 output_RAtest{i_TD}(i_elec, i_tonegroup, i_trial) = ...
%                         %                     RA_test(...
%                         %                     trialData{i_TD}(...
%                         %                     i_elec,...
        ...
        for i_TD = 1:length(ToneDur_text)
            for i_elec = 1:size(trialData{i_TD},1)
                for i_trial = 1:size(trialData{i_TD},3)                    
                    ...
                    trialData_detrend{i_TD}(i_elec,:,i_trial) = ...
                        detrend(trialData{i_TD}(i_elec,:,i_trial));
                    ...
                    trialData_tempnorm{i_TD}(i_elec,:,i_trial) = ...
                        trialData_detrend{i_TD}(i_elec,:,i_trial) - ...
                ...
                for i_trial = 1:size(trialData{i_TD},3)                    
                    trialData_ensemble{i_TD}(i_elec,:,i_trial) = ...
                        trialData_tempnorm{i_TD}(i_elec,:,i_trial) - ...
                        nanmean(squeeze(trialData_tempnorm{i_TD}(i_elec,:,:)),2)';
                end
                for i_trial = 1:size(trialData{i_TD},3)                   
                   trialData_ensemble2{i_TD}(i_elec,:,i_trial) = ...
                        trialData_ensemble{i_TD}(i_elec,:,i_trial) ./ ...
                        nanstd(squeeze(trialData{i_TD}(i_elec,:,:)),0,2)';
                end                
            end
        end
        
%         %Visual Check of data Preproc
%         %Plot single trial data trace for random electrode to visualize preprocessing
%         i_chan = randi(size(trialData{i_TD},1),1);
%         figure
%         subplot(2,2,1)
...
%         colorbar
%         title('Temporally normalized data')
%         subplot(2,2,4)
%         imagesc(squeeze(trialData_ensemble2{i_TD}(i_chan,:,:))')
%         colorbar
%         title('Ensemble normalized data')
%         %Plot trial-average data trace from random electrode to visualize preprocessing
%         figure
%         line(1:length(trialData{i_TD}),nanmean(trialData{i_TD}(i_chan,:,:),3),'Color','k')
%         hold on;
%         line(1:length(trialData_detrend{i_TD}),nanmean(trialData_detrend{i_TD}(i_chan,:,:),3),'Color','b')
%         line(1:length(trialData_tempnorm{i_TD}),nanmean(trialData_tempnorm{i_TD}(i_chan,:,:),3),'Color','r')
%         line(1:length(trialData_ensemble2{i_TD}),nanmean(trialData_ensemble2{i_TD}(i_chan,:,:),3),'Color','m')
%         legend('Raw', 'Detrend', 'TempNorm', 'EnsembleNorm')
        ...
        for i_TD = 1:length(ToneDur_text)
            ...
            param.GC.morder_pereffectTD(i_effect,i_TD) = floor(nanmedian(moBIC, 'all'));
            fprintf('\nbest model order (BIC) = %d\n',param.GC.morder_pereffectTD(i_effect,i_TD));
            
            %     %Optional: Sample autocovariance estimation
            %     G = tsdata_to_autocov(trialData_ensemble2{i_TD},param.GC.morder_pereffectTD(i_effect,i_TD))
                        
            num_Pairs{i_effect}           = 0;
            num_InvalidPairs{i_effect}    = 0;
                    ...
                    if ind_pairedelecs_from{i_effect}(i_sourceelec) ~= ind_pairedelecs_to{i_effect}(i_targetelec)
                        ...
                        for i_tonegroup = 1:size(param.GC.tone_aggregation,2)
                            ...
                            [AS.A, AS.SIG] = ...
                                ...
                                Sample_Tone_StartStop{i_TD}(param.GC.tone_aggregation{i_tonegroup}(1),1):...
                                Sample_Tone_StartStop{i_TD}(param.GC.tone_aggregation{i_tonegroup}(end),2), :), ...
                                param.GC.morder_pereffectTD(i_effect,i_TD), param.GC.regmode);
                            ...
                            validA{i_effect}{i_TD}(i_sourceelec, i_targetelec, i_tonegroup) = ...
                            ...
                            if validF{i_effect}{i_TD}(i_sourceelec, i_targetelec, i_tonegroup) == 1
                                ...
                                temporalGC{i_effect}{i_TD}.source2target...
                                    (i_sourceelec,i_targetelec,i_tonegroup) ...%(Source elec, Target elec, Tone)
                                    = FS.F(2,1);                                 
                                temporalGC_pval{i_effect}{i_TD}.source2target...
                                    (i_sourceelec,i_targetelec,i_tonegroup) ...%(Source elec, Target elec, Tone)
                                    = FS.pval(2,1);
                                frequencyGC{i_effect}{i_TD}.source2target...
                                ...
                                temporalGC{i_effect}{i_TD}.target2source...
                                    (i_sourceelec,i_targetelec,i_tonegroup) ...%(Target elec, Source elec, Tone)
                                    = FS.F(1,2);
                                temporalGC_pval{i_effect}{i_TD}.target2source...
                                    (i_sourceelec,i_targetelec,i_tonegroup) ...%(Target elec, Source elec, Tone)
                                    = FS.pval(1,2);
                                frequencyGC{i_effect}{i_TD}.target2source...
                                ...
                                num_Pairs{i_effect} = num_Pairs{i_effect} + 1;
                                
                            else %If GC computation didnt work out, substitute NaN values and count bailout
                                temporalGC{i_effect}{i_TD}.source2target...
                                    (i_sourceelec,i_targetelec,i_tonegroup) ...
                                    = nan;
                                temporalGC_pval{i_effect}{i_TD}.source2target...
                                ...
                                num_Pairs{i_effect} = ...
                                    num_Pairs{i_effect} + 1;
                                num_InvalidPairs{i_effect} = ...
                                    num_InvalidPairs{i_effect} + 1;
                            end                           
                        end
                    else %in case of double dipping, substitute NaN results but don't count as invalid
                        for i_tonegroup = 1:size(param.GC.tone_aggregation,2)
                            temporalGC{i_effect}{i_TD}.source2target...
                                (i_sourceelec,i_targetelec,i_tonegroup) ...
                                = nan;
            ...
            disp(['GC computation for ' ToneDur_text{i_TD} 's TD finished - ' ...
                num2str(num_Pairs{i_effect} - num_InvalidPairs{i_effect}) '/' ...
                num2str(num_Pairs{i_effect}) ' valid (GC computation did not bail out)']);
        end
    end
...
GCdata.info.epochdur_ms     = epochdur_ms;
