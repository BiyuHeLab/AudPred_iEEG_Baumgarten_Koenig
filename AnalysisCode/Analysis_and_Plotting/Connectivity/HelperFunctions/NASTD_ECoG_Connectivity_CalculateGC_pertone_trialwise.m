function GCdata = ...
    NASTD_ECoG_Connectivity_CalculateGC_pertone_trialwise ...
    (sub_list, i_sub, ...
...
for i_TD = 1:length(ToneDur_text)
    ...
    trial_IDs{i_TD} = DataClean_CleanTrialsElecs_perTD{i_TD}.trialinfo(:,1);
end
...
for i_effect = 1:length(param.GC.ElecPairEffect)
    if SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
        ...
        for i_TD = 1:length(ToneDur_text)
            ...
            temp_trialData{i_TD} = trialData{i_TD}(ind_pairedelecs_both, :, :);
        ...
        %     end
            ...
            validA{i_TD}          = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1)); %Indicates if VAR coefficients matrix is valid
            validF{i_TD}          = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1)); %Indicates if GC computation is valid

            temporalGC{i_TD}.source2target      = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1), size(Sample_Tone_StartStop{i_TD},1)); %pairwise-conditional time-domain multivariate Granger causalities
            temporalGC_pval{i_TD}.source2target = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1), size(Sample_Tone_StartStop{i_TD},1)); %pairwise-conditional frequency-domain/ spectral multivariate Granger causalites
            temporalGC{i_TD}.target2source      = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1), size(Sample_Tone_StartStop{i_TD},1)); %pairwise-conditional time-domain multivariate Granger causalities
            temporalGC_pval{i_TD}.target2source = nan(size(ind_pairedelecs_from{i_effect},1), size(ind_pairedelecs_to{i_effect},1), size(Sample_Tone_StartStop{i_TD},1)); %pairwise-conditional frequency-domain/ spectral multivariate Granger causalites
        ...
        for i_TD = 1:length(ToneDur_text)
            % Per tone and per trial
            num_tones = size(Sample_Tone_StartStop{i_TD}, 1);
            num_trials = size(trialData{i_TD}, 3);

            % Preallocate output
            temporalGC{i_TD}.source2target = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), num_tones, num_trials);
            temporalGC{i_TD}.target2source = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), num_tones, num_trials);
            temporalGC_pval{i_TD}.source2target = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), num_tones, num_trials);
            temporalGC_pval{i_TD}.target2source = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), num_tones, num_trials);
            if param.GC.downsample > 0
                frequencyGC{i_TD}.source2target = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), (param.GC.fs/param.GC.downsample)+1, num_tones, num_trials);
                frequencyGC{i_TD}.target2source = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), (param.GC.fs/param.GC.downsample)+1, num_tones, num_trials);
            else
                frequencyGC{i_TD}.source2target = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), (param.GC.fs)+1, num_tones, num_trials);
                frequencyGC{i_TD}.target2source = nan(length(ind_pairedelecs_from{i_effect}), length(ind_pairedelecs_to{i_effect}), (param.GC.fs)+1, num_tones, num_trials);
            end
                ...
                for i_targetelec = 1:length(ind_pairedelecs_to{i_effect})            
                    ...
                    if ind_pairedelecs_from{i_effect}(i_sourceelec) ~= ind_pairedelecs_to{i_effect}(i_targetelec)
                        ...
                        for i_tone = 1:size(Sample_Tone_StartStop{i_TD},1)
                            ...
                            try
                                [~, ~, moAIC(i_sourceelec, i_targetelec, i_tone), moBIC(i_sourceelec, i_targetelec, i_tone)] = ...
                                    tsdata_to_infocrit(...
                                    trialData{i_TD}([i_sourceelec, length(ind_pairedelecs_from{i_effect})+i_targetelec], ...
                                    Sample_Tone_StartStop{i_TD}(i_tone,1):Sample_Tone_StartStop{i_TD}(i_tone,2), :), ...
            ...
            for i_sourceelec = 1:length(ind_pairedelecs_from{i_effect})
                for i_targetelec = 1:length(ind_pairedelecs_to{i_effect})
                    %Avoid double-dipping (identical electrode as source + target)
                    if ind_pairedelecs_from{i_effect}(i_sourceelec) ~= ind_pairedelecs_to{i_effect}(i_targetelec)
                        disp(['-- Computing GC for ' ...
                            label_pairedelecs{i_sourceelec} ' -> ' ...
                            label_pairedelecs{length(ind_pairedelecs_from{i_effect}) + i_targetelec}]);

                        for i_tone = 1:num_tones
                            t_start = Sample_Tone_StartStop{i_TD}(i_tone,1);
                            t_end = Sample_Tone_StartStop{i_TD}(i_tone,2);
                            time_range = t_start:t_end;

                            for i_trial = 1:num_trials
                                try
                                    data_trial = trialData{i_TD}(...
                                        [i_sourceelec, length(ind_pairedelecs_from)+i_targetelec], ...
                                        time_range, i_trial);

                                    if size(data_trial,2) <= param.GC.morder
                                        continue;
                                    end

                                    [A, SIG] = tsdata_to_var(data_trial, param.GC.morder, param.GC.regmode);
                                    if isbad(A); continue; end

                                    [F, pval] = var_to_pwcgc(A, SIG, data_trial, param.GC.regmode, param.GC.tstat);
                                    if isbad(F, false); continue; end

                                    f_spect = var_to_spwcgc(A, SIG, param.GC.newfs);
                                    if isbad(f_spect, false); continue; end

                                    % Store results
                                    temporalGC{i_TD}.source2target(i_sourceelec, i_targetelec, i_tone, i_trial) = F(1,2);
                                    temporalGC{i_TD}.target2source(i_sourceelec, i_targetelec, i_tone, i_trial) = F(2,1);
                                    temporalGC_pval{i_TD}.source2target(i_sourceelec, i_targetelec, i_tone, i_trial) = pval(1,2);
                                    temporalGC_pval{i_TD}.target2source(i_sourceelec, i_targetelec, i_tone, i_trial) = pval(2,1);
                                    frequencyGC{i_TD}.source2target(i_sourceelec, i_targetelec, :, i_tone, i_trial) = f_spect(1,2,:);
                                    frequencyGC{i_TD}.target2source(i_sourceelec, i_targetelec, :, i_tone, i_trial) = f_spect(2,1,:);
                        ...
                        % Fill with NaNs for double-dipping
                        temporalGC{i_TD}.source2target(i_sourceelec, i_targetelec, :, :) = NaN;
                        temporalGC{i_TD}.target2source(i_sourceelec, i_targetelec, :, :) = NaN;
                        temporalGC_pval{i_TD}.source2target(i_sourceelec, i_targetelec, :, :) = NaN;
                        temporalGC_pval{i_TD}.target2source(i_sourceelec, i_targetelec, :, :) = NaN;
                        frequencyGC{i_TD}.source2target(i_sourceelec, i_targetelec, :, :, :) = NaN;
                        frequencyGC{i_TD}.target2source(i_sourceelec, i_targetelec, :, :, :) = NaN;
                    end
                end
            end
...
GCdata.info.validVAR = []; % Skipped per-trial validity for simplicity
GCdata.info.validGC = [];
...
GCdata.info.trialIDs = trial_IDs;
