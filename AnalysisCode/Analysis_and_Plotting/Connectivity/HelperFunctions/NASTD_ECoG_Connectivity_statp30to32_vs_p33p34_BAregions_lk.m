function NASTD_ECoG_Connectivity_statp30to32_vs_p33p34_BAregions_lk ...
    (subs, ...
    GCdata, ...
    ToneDur_text, ...
...
%for each effect type, respectively. Focused on comparisons between p32, p33, and p34.
%Apply paired statistical comparison (vs p32)
    ...
    for i_sub = 1:length(subs)
        if ~isempty(GCdata{i_sub}.temporalGC{i_effect})
            ...
            for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                ...
                % CHANGING THIS TO BRODMANNS AREA LABELS!
                temp_anatlabel_sourceelec2 = ...
                    cell2mat(SelElecs.([subfield_label1 'Effect']){temp_filt_elec,12}); % 12 corresponds to the BAlabel
                ...
                for i_targetelec = 1:temp_num_targetelecs
                    ...
                    anatlabel_targetelec{i_sub}{i_targetelec, i_sourceelec} = ...
                        ...
                        SelElecs.Pairs.(param.GC.ElecPairEffect{i_effect}).(...
                        param.GC.ElecPairRegion).Pairs_perelec.(...
                        temp_anatlabel_sourceelec){i_targetelec,12}); % 12 corresponds to BA label
                    anatlabel_targetelec{i_sub}{i_targetelec, i_sourceelec} = ...
                        strrep(...
                        anatlabel_targetelec{i_sub}{i_targetelec, i_sourceelec}, ...
                        ', ', '_');
...
%     end
    ...
    for i_sub = 1:length(subs)
        if ~isempty(GCdata{i_sub}.temporalGC{i_effect})
            n_source = length(GCdata{i_sub}.ind_pairedelecs_from{i_effect});
            anatreg{i_effect}.sourceelec_index{i_sub} = nan(n_source,1);
            anatreg{i_effect}.sourceelec_label{i_sub} = cell(n_source,1);
            
            for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                for i_anatlabels = 1:length(anatreg{i_effect}.label_unique)
    ...
    for i_anatlabels_source = 1:length(anatreg{i_effect}.label_unique)
        for i_anatlabels_target = 1:length(anatreg{i_effect}.label_unique)
    ...
    NumGC_peranatreg{i_effect} = ...
        zeros(length(anatreg{i_effect}.label_unique), ...
        length(anatreg{i_effect}.label_unique), ...
            ...
            for i_anatlabels_target = 1:length(anatreg{i_effect}.label_unique)
                ...
                for i_sub = 1:length(subs)
                    if ~isempty(GCdata{i_sub}.temporalGC{i_effect}) && ...
                        ...
                        for i_sourceelec = 1:length(GCdata{i_sub}.ind_pairedelecs_from{i_effect})
                            if ~isempty(anatreg{i_effect}.sourceelec_index{i_sub}) && ...
                                    any(anatreg{i_effect}.sourceelec_index{i_sub}(i_sourceelec) == ... 
                                ...
                                for i_targetelec = 1:size(anatreg{i_effect}.targetelec_index{i_sub},1) 
                                    if any(anatreg{i_effect}.targetelec_index{i_sub}(i_targetelec,i_sourceelec) == ...
                                        ...
                                        disp(['Found GC value for source loc [' anatreg{i_effect}.label_unique{i_anatlabels_source} ...
                                            '(' GCdata{i_sub}.label_allelecs{GCdata{i_sub}.ind_pairedelecs_from{i_effect}(i_sourceelec)} ')] ' ...
                                            'to target loc [' anatreg{i_effect}.label_unique{i_anatlabels_target}  '('...
                                            label_targetelec ')]' ...
                                            ' for sub: ' subs{i_sub} ...
                                            ', TD: ' num2str(i_TD)]);
...
%% Average p30 to p32 GC values, to then compare to p33 and to p34.
% Copy data into new array
GCscores = Gavg_tempGC_peranatreg;

region_labels = {'Auditory Cortex', 'IFC Dorsal', 'IFC Ventral', 'dlPFC', 'IPL', 'PMC', 'STS'};

conditions = {'Pred_Pred', 'PE_PE'};

toneLabels = {'30', '31', '32', '33', '34'};

% Temporary map for aggregation
gc_map = containers.Map();

for i_effect = 1:length(conditions)
    curr_data = GCscores.source2target{i_effect}; % 7x7x2 cell array
    for i_TD = 1:2
        for i_sourceelec = 1:7
            for i_targetelec = 1:7
                gc_values = curr_data{i_sourceelec, i_targetelec, i_TD};
                if ~isempty(gc_values)
                    % Add columns: condition index and tone duration
                    n_rows = size(gc_values, 1);
                    cond_column = repmat(i_effect, n_rows, 1);
                    td_column = repmat(i_TD, n_rows, 1);
                    enriched_values = [gc_values, cond_column, td_column]; % [N×7]

                    % Source-target key
                    src = region_labels{i_sourceelec};
                    tgt = region_labels{i_targetelec};
                    key = [src '->' tgt];

                    if isKey(gc_map, key)
                        gc_map(key) = [gc_map(key); enriched_values];  % Append
                    else
                        gc_map(key) = enriched_values;  % New entry
                    end
                end
            end
        end
...
% Convert map to GC_summary struct array
all_keys = keys(gc_map);

GC_summary = struct();  % empty struct

for i = 1:length(all_keys)
    key = all_keys{i};   % e.g. 'Auditory Cortex->STS'
    parts = split(key, '->');
    
    source_region = strtrim(parts{1});
    target_region = strtrim(parts{2});
    
    % Skip if source and target are the same (ignoring case and spaces)
    if strcmpi(strrep(source_region,' ','') , strrep(target_region,' ',''))
        continue
    end
    
    % Replace '->' with double underscore and replace spaces with single underscore
    safe_source = strrep(source_region, ' ', '_');
    safe_target = strrep(target_region, ' ', '_');
    
    field_name = [safe_source '_' safe_target];  %  underscore separator
    
    % Create dynamic field and assign data
    GC_summary.(field_name) = struct();
    GC_summary.(field_name).Source = source_region;
    GC_summary.(field_name).Target = target_region;
    GC_summary.(field_name).GCvalues = gc_map(key);  % N x 7 matrix
    GC_summary.(field_name).tone_labels = toneLabels; % your tone labels cell array
end

% Now for each anatomical pairing, average tones p30 to p32, then
% statistically test vs. p33 and vs. p34

GC_stats = struct(); % Will have one field per anatomical pairing with data, [1x2] if only one TD had enough data, or [2x2] if both did
pairs = fieldnames(GC_summary);

for p = 1:numel(pairs)
    pair_name = pairs{p};
    data = GC_summary.(pair_name).GCvalues;  % N x 7 matrix: cols 1-5 GC, 6 cond, 7 tone dur
    
    % Unique conditions and tone durations
    conditions_unique = unique(data(:,6));
    toneDurations_unique = unique(data(:,7));
    
    % Prepare a sub-struct for this pair
    GC_stats.(pair_name) = struct();
    
    for c = 1:numel(conditions_unique)
        cond_idx = conditions_unique(c);
        for td = 1:numel(toneDurations_unique)
            dur_val = toneDurations_unique(td);
            
            % Filter rows for this condition and tone duration
            idx = (data(:,6) == cond_idx) & (data(:,7) == dur_val);
            sub_data = data(idx, :);
            
            % Average GC over tones 30-32 (cols 1 to 3)
            mean_30_32 = mean(sub_data(:,1:3), 2);
            tone33 = sub_data(:,4);
            tone34 = sub_data(:,5);
            
            % Paired test: 30-32 average vs tone33
            if numel(mean_30_32) >= 2 && numel(tone33) >= 2
                [h33, p33, ~, stats33] = ttest(mean_30_32, tone33);
            else
                h33 = NaN; p33 = NaN; stats33 = [];
            end
            
            % Paired test: 30-32 average vs tone34
            if numel(mean_30_32) >= 2 && numel(tone34) >= 2
                [h34, p34, ~, stats34] = ttest(mean_30_32, tone34);
            else
                h34 = NaN; p34 = NaN; stats34 = [];
            end
            
            % Save results in struct
            GC_stats.(pair_name)(c, td).Condition = cond_idx;
            GC_stats.(pair_name)(c, td).ToneDuration = dur_val;
            GC_stats.(pair_name)(c, td).Mean_30_32 = mean_30_32;
            GC_stats.(pair_name)(c, td).Tone33 = tone33;
            GC_stats.(pair_name)(c, td).Tone34 = tone34;
            GC_stats.(pair_name)(c, td).ttest_30_32_vs_33 = struct('h', h33, 'p', p33, 'stats', stats33);
            GC_stats.(pair_name)(c, td).ttest_30_32_vs_34 = struct('h', h34, 'p', p34, 'stats', stats34);
        end
    end
end

% Summary of significant comparisons
filename = 'GC_anatregions_summarystats.txt';
fid = fopen(filename, 'w');

fprintf(fid, 'Significant results (p < 0.05):\n\n');

for p = 1:numel(pairs)
    pair_name = pairs{p};
    stat_array = GC_stats.(pair_name);
    
    for c = 1:size(stat_array,1)
        for td = 1:size(stat_array,2)
            entry = stat_array(c, td);
            
            if isempty(entry) || ~isfield(entry, 'ttest_30_32_vs_33') || isempty(entry.ttest_30_32_vs_33)
                continue;
            end
            
            % Extract p-values
            p33 = entry.ttest_30_32_vs_33.p;
            p34 = entry.ttest_30_32_vs_34.p;
            cond_name = conditions{entry.Condition};
            
            % Calculate mean differences (tone33 - mean_30_32) and (tone34 - mean_30_32)
            mean_30_32 = mean(entry.Mean_30_32);
            mean_tone33 = mean(entry.Tone33);
            mean_tone34 = mean(entry.Tone34);
            
            if ~isempty(p33) && isnumeric(p33) && p33 < 0.05
                change33 = mean_tone33 - mean_30_32;
                direction33 = 'no change';
                if change33 > 0
                    direction33 = 'increase';
                elseif change33 < 0
                    direction33 = 'decrease';
                end
                fprintf(fid, 'Pair: %s | Comparison: 30-32 vs 33 | Condition: %s | ToneDuration: %d | p = %.4f | GC %s\n', ...
                    pair_name, cond_name, entry.ToneDuration, p33, direction33);
            end
            
            if ~isempty(p34) && isnumeric(p34) && p34 < 0.05
                change34 = mean_tone34 - mean_30_32;
                direction34 = 'no change';
                if change34 > 0
                    direction34 = 'increase';
                elseif change34 < 0
                    direction34 = 'decrease';
                end
                fprintf(fid, 'Pair: %s | Comparison: 30-32 vs 34 | Condition: %s | ToneDuration: %d | p = %.4f | GC %s\n', ...
                    pair_name, cond_name, entry.ToneDuration, p34, direction34);
            end
        end
    end
end
...
% Gavg_tempGC_peranatreg = GCscores;
% param.GC.tone_aggregation2 = {303132, 33, 34};
% param.GC.tone_aggregation_label2 = '_303132_33_34';
% 
...
%     
%     f_sub1.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub1.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub1.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub1.XTickLabel = param.GC.tone_aggregation2;
...
%     %Reg1->Reg2, p303132 vs. p33
%     [p_R12_p3031322p33, h, stats_R12_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(2),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(2),i_TD}(:,2));
%     %Reg1->Reg2, p303132 vs. p34
%     [p_R12_p303132p34, h, stats_R12_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(2),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(2),i_TD}(:,3));
%     %Reg2->Reg1, p303132 vs. p33
%     [p_R21_p303132p33, h, stats_R21_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(1),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(1),i_TD}(:,2));
%     %Reg2->Reg1, p303132 vs. p34
%     [p_R21_p303132p34, h, stats_R21_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(1),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(1),i_TD}(:,3));
%     
%     hold on;
%     text(1.3, f_sub1.YLim(2)*0.75, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R12_p303132p33.zval,3)) '; p = ' num2str(round(p_R12_p3031322p33,3))]}, ...
%         'Color',[0 0 1])
%     text(1.3, f_sub1.YLim(2)*0.25, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R21_p303132p33.zval,3)) '; p = ' num2str(round(p_R21_p303132p33,3))]}, ...
%         'Color',[1 0 0])
%     text(2.3, f_sub1.YLim(2)*0.75, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R12_p303132p34.zval,3)) '; p = ' num2str(round(p_R12_p303132p34,3))]}, ...
%         'Color',[0 0 1])
%     text(2.3, f_sub1.YLim(2)*0.25, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R21_p303132p34.zval,3)) '; p = ' num2str(round(p_R21_p303132p34,3))]}, ...
%         'Color',[1 0 0])
%     
%     legend([anatreg{i_effect}.label_unique{temp_currfilt(1)} ' -> ' ...
%         anatreg{i_effect}.label_unique{temp_currfilt(2)} ...
%         ' (' num2str(NumGC_peranatreg{i_effect}...
%         (temp_currfilt(1),temp_currfilt(2))) ' elec pairs)'], ...
%         [anatreg{i_effect}.label_unique{temp_currfilt(2)} ' -> ' ...
%         anatreg{i_effect}.label_unique{temp_currfilt(1)} ...
%         ' (' num2str(NumGC_peranatreg{i_effect}...
...
%     
%     f_sub2.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub2.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub2.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub2.XTickLabel = param.GC.tone_aggregation2;
%     a = get(gca, 'XTickLabel');
%     set(gca,'XTickLabel', a, 'fontsize', 8);
...
%     
%     %Wilcoxon signed rank test
%     %Reg1->Reg3, p303132 vs. p33
%     [p_R13_p303132p33, h, stats_R13_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(3),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(3),i_TD}(:,2));
%     %Reg1->Reg3, p303132 vs. p34
%     [p_R13_p303132p34, h, stats_R13_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(3),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(3),i_TD}(:,3));
%     %Reg2->Reg1, p303132 vs. p33
%     [p_R31_p303132p33, h, stats_R31_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(1),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(1),i_TD}(:,2));
%     %Reg2->Reg1, p303132 vs. p34
%     [p_R31_p303132p34, h, stats_R31_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(1),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(1),i_TD}(:,3));
%     
%     hold on;
%     text(1.3, f_sub1.YLim(2)*0.75, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R13_p303132p33.zval,3)) '; p = ' num2str(round(p_R13_p303132p33,3))]}, ...
%         'Color',[0 0 1])
%     text(1.3, f_sub1.YLim(2)*0.25, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R31_p303132p33.zval,3)) '; p = ' num2str(round(p_R31_p303132p33,3))]}, ...
%         'Color',[1 0 0])
%     text(2.3, f_sub1.YLim(2)*0.75, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R13_p303132p34.zval,3)) '; p = ' num2str(round(p_R13_p303132p34,3))]}, ...
%         'Color',[0 0 1])
%     text(2.3, f_sub1.YLim(2)*0.25, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R31_p303132p34.zval,3)) '; p = ' num2str(round(p_R31_p303132p34,3))]}, ...
%         'Color',[1 0 0])
%     
%     legend([anatreg{i_effect}.label_unique{temp_currfilt(1)} ' -> ' ...
%         anatreg{i_effect}.label_unique{temp_currfilt(3)} ...
%         ' (' num2str(NumGC_peranatreg{i_effect}...
...
%     
%     f_sub3.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub3.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub3.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub3.XTickLabel = param.GC.tone_aggregation2;
%     a = get(gca, 'XTickLabel');
%     set(gca,'XTickLabel', a, 'fontsize', 8);
...
%     
%     %Wilcoxon signed rank test
%     %Reg2->Reg3, p303132 vs. p33
%     [p_R23_p303132p33, h, stats_R23_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(3),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(3),i_TD}(:,2));
%     %Reg2->Reg3, p303132 vs. p34
%     [p_R23_p303132p34, h, stats_R23_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(3),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(2),temp_currfilt(3),i_TD}(:,3));
%     %Reg3->Reg2, p303132 vs. p33
%     [p_R32_p303132p33, h, stats_R32_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(2),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(2),i_TD}(:,2));
%     %Reg3->Reg2, p303132 vs. p34
%     [p_R32_p303132p34, h, stats_R32_p303132p34] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(2),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(3),temp_currfilt(2),i_TD}(:,3));
%     
%     hold on;
%     text(1.3, f_sub1.YLim(2)*0.75, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R23_p303132p33.zval,3)) '; p = ' num2str(round(p_R23_p303132p33,3))]}, ...
%         'Color',[0 0 1])
%     text(1.3, f_sub1.YLim(2)*0.25, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R32_p303132p33.zval,3)) '; p = ' num2str(round(p_R32_p303132p33,3))]}, ...
%         'Color',[1 0 0])
%     text(2.3, f_sub1.YLim(2)*0.75, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R23_p303132p34.zval,3)) '; p = ' num2str(round(p_R23_p303132p34,3))]}, ...
%         'Color',[0 0 1])
%     text(2.3, f_sub1.YLim(2)*0.25, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R32_p303132p34.zval,3)) '; p = ' num2str(round(p_R32_p303132p34,3))]}, ...
%         'Color',[1 0 0])
%     
%     legend([anatreg{i_effect}.label_unique{temp_currfilt(2)} ' -> ' ...
%         anatreg{i_effect}.label_unique{temp_currfilt(3)} ...
%         ' (' num2str(NumGC_peranatreg{i_effect}...
...
%     
%     f_sub4.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub4.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub4.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub4.XTickLabel = param.GC.tone_aggregation2;
%     a = get(gca, 'XTickLabel');
%     set(gca,'XTickLabel', a, 'fontsize', 8);
...
%     %Wilcoxon signed rank test
%     %Reg1->Reg2, p303132 vs. p33
%     [p_R12_p303132p33, h, stats_R12_p303132p33] = ...
%         signrank(...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
%         {temp_currfilt(1),temp_currfilt(2),i_TD}(:,1), ...
%         Gavg_tempGC_peranatreg.source2target{i_effect}...
...
%     
%     hold on;
%     text(1.3, f_sub1.YLim(2)*0.65, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R12_p303132p33.zval,3)) '; p = ' num2str(round(p_R12_p303132p33,3))]}, ...
%         'Color',[0 0 1])
%     text(1.3, f_sub1.YLim(2)*0.25, ...
%         {'p33vsp303132', ['z = ' num2str(round(stats_R21_p303132p33.zval,3)) '; p = ' num2str(round(p_R21_p303132p33,3))]}, ...
%         'Color',[1 0 0])
%     text(2.3, f_sub1.YLim(2)*0.65, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R12_p303132p34.zval,3)) '; p = ' num2str(round(p_R12_p303132p34,3))]}, ...
%         'Color',[0 0 1])
%     text(2.3, f_sub1.YLim(2)*0.25, ...
%         {'p34vsp303132', ['z = ' num2str(round(stats_R21_p303132p34.zval,3)) '; p = ' num2str(round(p_R21_p303132p34,3))]}, ...
...
%     
%     f_sub5.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub5.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub5.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub5.XTickLabel = param.GC.tone_aggregation2;
%     a = get(gca, 'XTickLabel');
%     set(gca,'XTickLabel', a, 'fontsize', 8);
...
%     
%     f_sub6.XLim  = [1-0.3 length(param.GC.tone_aggregation2)+0.3];
%     if istrue(Yaxis_scaling)
%         f_sub6.YLim  = [Ylim_temp(1) Ylim_temp(2)];
%     end
%     f_sub6.XTick =  [1:length(param.GC.tone_aggregation2)];
%     f_sub6.XTickLabel = param.GC.tone_aggregation2;
%     a = get(gca, 'XTickLabel');
%     set(gca,'XTickLabel', a, 'fontsize', 8);
...
%             param.GC.InputDataType{1} param.GC.tone_aggregation_label2 '_ensemnorm_stat.png'];
...
%             'Color',[0 0 1])
%         text(2.3, f_sub1.YLim(2)*0.25, ...
%             {'p34vsp32', ['z = ' num2str(round(stats_R32_p303132p34.zval,3)) '; p = ' num2str(round(p_R32_p303132p34,3))]}, ...
%             'Color',[1 0 0]) 
%         
...
%         
%         hold on;
%         text(1.3, f_sub4.YLim(2)*0.65, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R12_p303132p33.zval,3)) '; p = ' num2str(round(p_R12_p303132p33,3))]}, ...
%             'Color',[0 0 1])
%         text(1.3, f_sub4.YLim(2)*0.25, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R21_p303132p33.zval,3)) '; p = ' num2str(round(p_R21_p303132p33,3))]}, ...
%             'Color',[1 0 0])
%         text(2.3, f_sub4.YLim(2)*0.65, ...
%              {'p34vsp303132', ['z = ' num2str(round(stats_R12_p303132p34.zval,3)) '; p = ' num2str(round(p_R12_p303132p34,3))]}, ...
%             'Color',[0 0 1])
%         text(2.3, f_sub4.YLim(2)*0.25, ...
%             {'p34vsp303132', ['z = ' num2str(round(stats_R21_p303132p34.zval,3)) '; p = ' num2str(round(p_R21_p303132p34,3))]}, ...
%             'Color',[1 0 0])        
% 
%         legend([anatreg{i_effect}.label_aggregated{temp_currfilt(1)} ' -> ' ...
...
%         
%         hold on;
%         text(1.3, f_sub5.YLim(2)*0.65, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R13_p303132p33.zval,3)) '; p = ' num2str(round(p_R13_p303132p33,3))]}, ...
%             'Color',[0 0 1])
%         text(1.3, f_sub5.YLim(2)*0.25, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R31_p303132p33.zval,3)) '; p = ' num2str(round(p_R31_p303132p33,3))]}, ...
%             'Color',[1 0 0])
%         text(2.3, f_sub5.YLim(2)*0.65, ...
%              {'p34vsp303132', ['z = ' num2str(round(stats_R13_p303132p34.zval,3)) '; p = ' num2str(round(p_R13_p303132p34,3))]}, ...
%             'Color',[0 0 1])
%         text(2.3, f_sub5.YLim(2)*0.25, ...
%             {'p34vsp303132', ['z = ' num2str(round(stats_R31_p303132p34.zval,3)) '; p = ' num2str(round(p_R31_p303132p34,3))]}, ...
%             'Color',[1 0 0])                
% 
%         legend([anatreg{i_effect}.label_aggregated{temp_currfilt(1)} ' -> ' ...
...
%         
%         hold on;
%         text(1.3, f_sub6.YLim(2)*0.65, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R23_p303132p33.zval,3)) '; p = ' num2str(round(p_R23_p303132p33,3))]}, ...
%             'Color',[0 0 1])
%         text(1.3, f_sub6.YLim(2)*0.25, ...
%             {'p33vsp303132', ['z = ' num2str(round(stats_R32_p303132p33.zval,3)) '; p = ' num2str(round(p_R32_p303132p33,3))]}, ...
%             'Color',[1 0 0])
%         text(2.3, f_sub6.YLim(2)*0.65, ...
%              {'p34vsp303132', ['z = ' num2str(round(stats_R23_p303132p34.zval,3)) '; p = ' num2str(round(p_R23_p303132p34,3))]}, ...
%             'Color',[0 0 1])
%         text(2.3, f_sub6.YLim(2)*0.25, ...
%             {'p34vsp303132', ['z = ' num2str(round(stats_R32_p303132p34.zval,3)) '; p = ' num2str(round(p_R32_p303132p34,3))]}, ...
%             'Color',[1 0 0])        
%        
%         legend([anatreg{i_effect}.label_aggregated{temp_currfilt(2)} ' -> ' ...
...
%             param.GC.InputDataType{1} param.GC.tone_aggregation_label '_ensemnorm_stat.png'];
%         figfile      = [path_fig  filename];
%         saveas(gcf, figfile, 'png'); %save png version
%         close;
%     end    
