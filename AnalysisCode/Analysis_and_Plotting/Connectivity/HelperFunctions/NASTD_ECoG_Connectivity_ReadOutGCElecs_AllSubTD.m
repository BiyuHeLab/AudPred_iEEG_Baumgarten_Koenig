function SelElecs = NASTD_ECoG_Connectivity_ReadOutGCElecs_AllSubTD...
    (subs, ...
...
varnames = ...
    {'ElectrodeLabel', 'SubjectLabel', 'InputSignal', 'TD_of_effect', ...
    'AnatCatLabel', 'DetailedLabel', 'ElecCoords', ...
    'Effect_pvalue', 'ElecIndex_all_elecs', 'ElecIndex_Ssub_elecs', ...
    'Cluster_Timecourse'};
...
%% 4) Determine possible pairings between sign. electrodes
%Determine pairings between electrodes with sign effects for the same
%subject and specific spatial (frontal - temporal) and effect (Pred - PE)
%combinations.
...
if plot_poststepFigs == 1
    ...
    %% 4.1 Pred - Pred Electrodes
    %All regions
    SelElecs.Pairs.Pred_Pred.AllRegions.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PredEffect)
        %Determine current elec information
        temp_label  = SelElecs.PredEffect{i_elec,1};
        temp_sub    = SelElecs.PredEffect{i_elec,2};
        temp_anatcat = SelElecs.PredEffect{i_elec,5};
        %Find further sign. elecs from same subject
        temp_index_otherelecs = ...
            find(strcmp(temp_sub, SelElecs.PredEffect{:,2}) & ...
            ~strcmp(temp_label, SelElecs.PredEffect{:,1}));
        %Create label for subfield
        ...
        %Store pair information in new struct
        if ~isempty(temp_index_otherelecs)
            SelElecs.Pairs.Pred_Pred.AllRegions.Pairs_perelec.(temp_fieldname) = ...
    ...
    for i_sub = 1:length(subs)
        sub = subs(i_sub);
        index_sign_elecs_currsub = find(strcmp(sub, SelElecs.PredEffect{:,2}));
        ...
        if size(temp_pairs1,1) > 1
            ...
            SelElecs.Pairs.Pred_Pred.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.Pred_Pred.AllRegions.Ind_UniquePairings_persub{i_sub} = ...
    ...
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - Pred to Pred (' folderlabel_inputsignal ') - All regions'])
    for i_sub = 1:length(subs)
        
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_Pred.AllRegions.Ind_UniquePairings_persub{i_sub})
                curr_elec_ind = unique(SelElecs.Pairs.Pred_Pred.AllRegions.Ind_UniquePairings_persub{i_sub});
            else
                curr_elec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
            end
            ...
            elec_pairings = SelElecs.Pairs.Pred_Pred.AllRegions.Ind_UniquePairings_persub{i_sub};
            ...
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(size(elec_coords,1)) ' Pred elecs - ' ...
                num2str(SelElecs.Pairs.Pred_Pred.AllRegions.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
            ...
            subplot(DimSubplot(1), DimSubplot(2), i_sub);
            title([subs{i_sub} ' - ' '0 sig. pred elecs']);
            axis off
        end
    end
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_Pred2Pred_AllReg_'  ...
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - Pred to Pred (' folderlabel_inputsignal ') - {\color{blue}Frontal} to {\color{red}Temporal}'])
    for i_sub = 1:length(subs)
        
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings_persub{i_sub})
                curr_elec_ind = unique(SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings_persub{i_sub});
            else
                curr_elec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
            end
            elec_coords = SelElecs.PredEffect{curr_elec_ind,7};
            ...
            elec_typeindex(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind,5}, 'AntPFC'))) = 1; %1 = source, 2 = target
            elec_typeindex(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind,5}, 'VentralT'))) = 2; %1 = source, 2 = target
            elec_pairings = SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'Pred_Pred';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            ...
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(size(elec_coords,1)) ' Pred elecs - ' ...
                num2str(SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
            ...
            subplot(DimSubplot(1), DimSubplot(2), i_sub);
            title([subs{i_sub} ' - ' '0 pred elecs']);
            axis off
        end
    end
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_Pred2Pred_Front2Temp_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
    ...
    %% 4.2 PE - PE
    %All
    SelElecs.Pairs.PE_PE.AllRegions.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PEEffect)
        %Determine current elec information
        temp_label  = SelElecs.PEEffect{i_elec,1};
        temp_sub    = SelElecs.PEEffect{i_elec,2};
        temp_anatcat = SelElecs.PEEffect{i_elec,5};
        %Find further sign. elecs from same subject
        temp_index_otherelecs = ...
            find(strcmp(temp_sub, SelElecs.PEEffect{:,2}) & ...
        ...
        if ~isempty(temp_index_otherelecs)
            %Store pair information in new struct
            SelElecs.Pairs.PE_PE.AllRegions.Pairs_perelec.(temp_fieldname) = ...
    ...
    for i_sub = 1:length(subs)
        sub = subs(i_sub);
        index_sign_elecs_currsub = find(strcmp(sub, SelElecs.PEEffect{:,2}));
        ...
        if size(temp_pairs1,1) > 1
            ...
            SelElecs.Pairs.PE_PE.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.PE_PE.AllRegions.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs2(~i_doubles,:);
        elseif size(temp_pairs1,1) == 1 && temp_pairs1(1) ~= temp_pairs1(2)
            SelElecs.Pairs.PE_PE.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                1;
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - PE to PE (' folderlabel_inputsignal ') - All regions'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}))
            if ~isnan(SelElecs.Pairs.PE_PE.AllRegions.Ind_UniquePairings_persub{i_sub})
                curr_elec_ind = unique(SelElecs.Pairs.PE_PE.AllRegions.Ind_UniquePairings_persub{i_sub});
            else
                curr_elec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
            end
            ...
            elec_signindex = true(size(elec_coords,1),1);
            elec_typeindex = [ones(length(curr_elec_ind),1) * 3]; %1 = source, 2 = target, 3 = both
            elec_pairings = SelElecs.Pairs.PE_PE.AllRegions.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'PE_PE';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(size(elec_coords,1)) ' PE elecs - ' ...
                num2str(SelElecs.Pairs.PE_PE.AllRegions.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
            ...
            subplot(DimSubplot(1), DimSubplot(2), i_sub);
            title([subs{i_sub} ' - ' '0 PE elecs']);
            axis off
        end
    end
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_PE2PE_AllReg_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
    ...
    %Temporal - Frontal
    SelElecs.Pairs.PE_PE.Temporal_Frontal.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PEEffect)
        %Determine current elec information
        temp_label  = SelElecs.PEEffect{i_elec,1};
        temp_sub    = SelElecs.PEEffect{i_elec,2};
        temp_anatcat = SelElecs.PEEffect{i_elec,5};
        if strfind(temp_anatcat{1}, 'VentralT') %If current electrode is temporal
            
            temp_index_otherelecs = ...%Find other sign. elecs from same subject in frontal areas
                find(strcmp(temp_sub, SelElecs.PEEffect{:,2}) & ...
                ~strcmp(temp_label, SelElecs.PEEffect{:,1}) & ...
            ...
            if ~isempty(temp_index_otherelecs) %Store pair information in new struct
                SelElecs.Pairs.PE_PE.Temporal_Frontal.Pairs_perelec.(temp_fieldname) = ...
                    SelElecs.PEEffect(temp_index_otherelecs,:);
            else
                SelElecs.Pairs.PE_PE.Temporal_Frontal.Pairs_perelec.(temp_fieldname) = [];
            end
    ...
    for i_sub = 1:length(subs)
        ...
        if size(temp_pairs1,1) > 1
            ...
            SelElecs.Pairs.PE_PE.Temporal_Frontal.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.PE_PE.Temporal_Frontal.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs2(~i_doubles,:);
        elseif size(temp_pairs1,1) == 1 && temp_pairs1(1) ~= temp_pairs1(2)
            SelElecs.Pairs.PE_PE.Temporal_Frontal.Num_UniquePairings_persub(1,i_sub) = ...
                1;
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - PE to PE (' folderlabel_inputsignal ') - {\color{blue}Temporal} to {\color{red}Frontal}'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}))
            if ~isnan(SelElecs.Pairs.PE_PE.Temporal_Frontal.Ind_UniquePairings_persub{i_sub})
                curr_elec_ind = unique(SelElecs.Pairs.PE_PE.Temporal_Frontal.Ind_UniquePairings_persub{i_sub});
            else
                curr_elec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
            end
            elec_coords = SelElecs.PEEffect{curr_elec_ind,7};
            ...
            elec_signindex = true(size(elec_coords,1),1);
            elec_typeindex = ones(size(elec_coords,1),1)*3;
            elec_typeindex(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind,5}, 'VentralT'))) = 1; %1 = source, 2 = target
            elec_typeindex(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind,5}, 'AntPFC'))) = 2; %1 = source, 2 = target
            elec_pairings = SelElecs.Pairs.PE_PE.Temporal_Frontal.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'PE_PE';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(size(elec_coords,1)) ' PE elecs - ' ...
                num2str(SelElecs.Pairs.PE_PE.Temporal_Frontal.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
    ...
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_PE2PE_Temp2Front_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
    ...
    %% 4.3 Pred - PE
    %All
    SelElecs.Pairs.Pred_PE.AllRegions.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PredEffect)
        %Determine current elec information
        temp_label  = SelElecs.PredEffect{i_elec,1};
        temp_sub    = SelElecs.PredEffect{i_elec,2};
        temp_anatcat = SelElecs.PredEffect{i_elec,5};
        %Find other sign. PE elecs from same subject
        temp_index_otherelecs = ...
            find(strcmp(temp_sub, SelElecs.PEEffect{:,2}) & ...
            ~strcmp(temp_label, SelElecs.PEEffect{:,1}));
        if ~isempty(strfind(temp_anatcat{1}, ','))%Create label for subfield
        ...
        if ~isempty(temp_index_otherelecs)
            %Store pair information in new struct
            SelElecs.Pairs.Pred_PE.AllRegions.Pairs_perelec.(temp_fieldname) = ...
                SelElecs.PEEffect(temp_index_otherelecs,:);
        else
            SelElecs.Pairs.Pred_PE.AllRegions.Pairs_perelec.(temp_fieldname) = [];
        end
    ...
    for i_sub = 1:length(subs)
        sub = subs(i_sub);
        sign_elecs_currsub1 = find(strcmp(sub, SelElecs.PredEffect{:,2}));
        sign_elecs_currsub2 = find(strcmp(sub, SelElecs.PEEffect{:,2}));
        %Determine number of possible pairings for each subject
        [p,q] = meshgrid(sign_elecs_currsub1, sign_elecs_currsub2);
        temp_pairs1 = [p(:) q(:)];
        if size(temp_pairs1,1) > 1
            %delete same-same pairings (here sameness determined via name, since Pred/PE indexing different)
            temp_pairs2 = [];
            for i_pairs = 1:length(temp_pairs1)
                label_sourceelec = SelElecs.PredEffect{temp_pairs1(i_pairs,1),1};
                index_equivalentelec = strcmp(label_sourceelec, SelElecs.PEEffect{:,1}); %filter showing target elecs same as sourceelec
                if index_equivalentelec(temp_pairs1(i_pairs,2)) == false %if target elec not same as source elec
                    temp_pairs2 = [temp_pairs2; temp_pairs1(i_pairs,:)]; %copy pairing
                end
            end
            %delete doubles (can be checked independent of different Pred/PE indexing)
            i_doubles = false(length(temp_pairs2),1);
            i_compared = false(length(temp_pairs2),1);
            for i_pairs = 1:length(temp_pairs2)
                temp_doubles = find(all(temp_pairs2(i_pairs,:) == fliplr(temp_pairs2),2));
            ...
            SelElecs.Pairs.Pred_PE.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs2(~i_doubles,:);
        elseif size(temp_pairs1,1) == 1
            SelElecs.Pairs.Pred_PE.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                1;
            SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs1;
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - {\color{blue}Pred} to {\color{red}PE} (' folderlabel_inputsignal ') - All regions'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            %Determine pairings or, if not present, sign. elecs
            if ~isnan(SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub})
                curr_Predelec_ind = unique(SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub}(:,1));
                curr_PEelec_ind = unique(SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub}(:,2));
            else
                curr_Predelec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
                curr_PEelec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
            end
            ...
            %Check if sign. elecs have both Pred and PE effect
            filt_elecs_botheffects = false(length(elec_labels),1); %index for coloring
            for i_elec = 1:length(elec_labels)
                filt_elecs_botheffects(i_elec,1)  = ...
                    any(strcmp(elec_labels{i_elec}, SelElecs.PredPEEffect{:,1}) & ...
                    strcmp(subs{i_sub}, SelElecs.PredPEEffect{:,2}));
            end
            num_elecs_botheffects = 0; %number for counting
            temp_uniqueeleclabels = unique(elec_labels);
            for i_elec = 1:length(temp_uniqueeleclabels)
                num_elecs_botheffects = num_elecs_botheffects + ...
                    sum(strcmp(temp_uniqueeleclabels{i_elec}, SelElecs.PredPEEffect{:,1}) & ...
                    strcmp(subs(i_sub),SelElecs.PredPEEffect{:,2}));
            end
            elec_data = ones(size(elec_coords,1),1)*i_sub;
            sub_index = ones(size(elec_coords,1),1)*i_sub;
            elec_signindex = true(size(elec_coords,1),1);
            elec_typeindex = [ones(length(curr_Predelec_ind),1); ones(length(curr_PEelec_ind),1)*2]; %1 = source, 2 = target, 3 = none, 4 = both
            elec_typeindex(filt_elecs_botheffects) = 4;
            elec_pairings = SelElecs.Pairs.Pred_PE.AllRegions.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'Pred_PE';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(length(curr_Predelec_ind)) ' Pred Elecs / '  ...
                num2str(length(curr_PEelec_ind)) ' PE elecs / ' num2str(num_elecs_botheffects) ' Pred+PE elecs - '...
                num2str(SelElecs.Pairs.Pred_PE.AllRegions.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
            ...
            subplot(DimSubplot(1), DimSubplot(2), i_sub);
            title([subs{i_sub} ' - ' '0 elec pairs']);
            axis off
        end
    end
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_Pred2PE_AllReg_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
    ...
    for i_elec = 1:height(SelElecs.PredEffect)
        ...
        if strfind(temp_anatcat{1}, 'AntPFC') %If current electrode is Frontal
            ...
            temp_index_otherelecs = ...%Find other sign. PE elecs from same subject in temporal areas
                find(strcmp(temp_sub, SelElecs.PEEffect{:,2}) & ...
                ~strcmp(temp_label, SelElecs.PEEffect{:,1}) & ...
                ~cellfun('isempty',strfind(SelElecs.PEEffect{:,5}, 'VentralT')));
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - {\color{blue}Pred} to {\color{red}PE} (' folderlabel_inputsignal ') - {\color{blue}Frontal} to {\color{red}Temporal}'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_PE.Frontal_Temporal.Ind_UniquePairings_persub{i_sub})
                curr_Predelec_ind = unique(SelElecs.Pairs.Pred_PE.Frontal_Temporal.Ind_UniquePairings_persub{i_sub}(:,1));
                curr_PEelec_ind = unique(SelElecs.Pairs.Pred_PE.Frontal_Temporal.Ind_UniquePairings_persub{i_sub}(:,2));
            else
                curr_Predelec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
                curr_PEelec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
            end
            ...
            sub_index = ones(size(elec_coords,1),1)*i_sub;
            elec_signindex = true(size(elec_coords,1),1);
            elec_typeindex = [ones(length(curr_Predelec_ind),1); ones(length(curr_PEelec_ind),1)*2]; %1 = source, 2 = target
            elec_typeindex(filt_elecs_botheffects) = 4;
            elec_pairings = SelElecs.Pairs.Pred_PE.Frontal_Temporal.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'Pred_PE';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            ...
            sp_title = {[subs{i_sub} ' - ' num2str(length(curr_Predelec_ind)) ' Pred Elecs / '  ...
                num2str(length(curr_PEelec_ind)) ' PE elecs / ' num2str(num_elecs_botheffects) ' Pred+PE elecs - '...
                num2str(SelElecs.Pairs.Pred_PE.Frontal_Temporal.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
    ...
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_Pred2PE_Front2Temp_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
    ...
    %% 4.4 PE - Pred
    %All
    SelElecs.Pairs.PE_Pred.AllRegions.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PEEffect)
        %Determine current elec information
        temp_label  = SelElecs.PEEffect{i_elec,1};
        temp_sub    = SelElecs.PEEffect{i_elec,2};
        temp_anatcat = SelElecs.PEEffect{i_elec,5};
        %Find other sign. prediction elecs from same subject
        temp_index_otherelecs = ...
            find(strcmp(temp_sub, SelElecs.PredEffect{:,2}) & ...
            ~strcmp(temp_label, SelElecs.PredEffect{:,1}));
        if ~isempty(strfind(temp_anatcat{1}, ','))%Create label for subfield
        ...
        if ~isempty(temp_index_otherelecs)
            %Store pair information in new struct
            SelElecs.Pairs.PE_Pred.AllRegions.Pairs_perelec.(temp_fieldname) = ...
                SelElecs.PredEffect(temp_index_otherelecs,:);
        else
            SelElecs.Pairs.PE_Pred.AllRegions.Pairs_perelec.(temp_fieldname) = [];
        end
    ...
    for i_sub = 1:length(subs)
        sub = subs(i_sub);
        sign_elecs_currsub1 = find(strcmp(sub, SelElecs.PEEffect{:,2}));
        sign_elecs_currsub2 = find(strcmp(sub, SelElecs.PredEffect{:,2}));
        ...
        if size(temp_pairs1,1) > 1
            ...
            temp_pairs2 = [];
            for i_pairs = 1:length(temp_pairs1)
                label_sourceelec = SelElecs.PEEffect{temp_pairs1(i_pairs,1),1};
                index_equivalentelec = strcmp(label_sourceelec, SelElecs.PredEffect{:,1}); %filter showing target elecs same as sourceelec
                if index_equivalentelec(temp_pairs1(i_pairs,2)) == false %if target elec not same as source elec
                    temp_pairs2 = [temp_pairs2; temp_pairs1(i_pairs,:)]; %copy pairing
                end
            end
            ...
            SelElecs.Pairs.PE_Pred.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.PE_Pred.AllRegions.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs2(~i_doubles,:);
        elseif size(temp_pairs1,1) == 1
            SelElecs.Pairs.PE_Pred.AllRegions.Num_UniquePairings_persub(1,i_sub) = ...
                1;
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - {\color{blue}PE} to {\color{red}Pred} (p < ' num2str(param.pval_plotting) ', ' FDR_label ') - All regions'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}))
            if ~isnan(SelElecs.Pairs.PE_Pred.AllRegions.Ind_UniquePairings_persub{i_sub})
                curr_PEelec_ind = unique(SelElecs.Pairs.PE_Pred.AllRegions.Ind_UniquePairings_persub{i_sub}(:,1));
                curr_Predelec_ind = unique(SelElecs.Pairs.PE_Pred.AllRegions.Ind_UniquePairings_persub{i_sub}(:,2));
            else
                curr_PEelec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
                curr_Predelec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
            end
            ...
            sub_index = ones(size(elec_coords,1),1)*i_sub;
            elec_signindex = true(size(elec_coords,1),1);
            elec_typeindex = [ones(length(curr_PEelec_ind),1); ones(length(curr_Predelec_ind),1)*2]; %1 = source, 2 = target
            elec_typeindex(filt_elecs_botheffects) = 4;
            elec_pairings = SelElecs.Pairs.PE_Pred.AllRegions.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'PE_Pred';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            textcolor_rgb   = [0 0 0];
            sp_title = {[subs{i_sub} ' - ' num2str(length(curr_PEelec_ind)) ' PE Elecs / '  ...
                num2str(length(curr_Predelec_ind)) ' Pred elecs / ' num2str(num_elecs_botheffects) ' Pred+PE elecs - '...
                num2str(SelElecs.Pairs.PE_Pred.AllRegions.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
    ...
    %Temporal - Frontal
    SelElecs.Pairs.PE_Pred.Temporal_Frontal.Pairs_perelec = struct;
    for i_elec = 1:height(SelElecs.PEEffect)
        %Determine current elec information
        temp_label  = SelElecs.PEEffect{i_elec,1};
        temp_sub    = SelElecs.PEEffect{i_elec,2};
        ...
        if strfind(temp_anatcat{1}, 'VentralT') %If current electrode is temporal
            ...
            temp_index_otherelecs = ...%Find other sign. prediction elecs from same subject in frontal areas
                find(strcmp(temp_sub, SelElecs.PredEffect{:,2}) & ...
                ~strcmp(temp_label, SelElecs.PredEffect{:,1}) & ...
                ~cellfun('isempty',strfind(SelElecs.PredEffect{:,5}, 'AntPFC')));
            ...
            if ~isempty(temp_index_otherelecs)
                %Store pair information in new struct
                SelElecs.Pairs.PE_Pred.Temporal_Frontal.Pairs_perelec.(temp_fieldname) = ...
                    SelElecs.PredEffect(temp_index_otherelecs,:);
            else
                SelElecs.Pairs.PE_Pred.Temporal_Frontal.Pairs_perelec.(temp_fieldname) = [];
            end
    ...
    for i_sub = 1:length(subs)
        ...
        if size(temp_pairs1,1) > 1
            ...
            SelElecs.Pairs.PE_Pred.Temporal_Frontal.Num_UniquePairings_persub(1,i_sub) = ...
                size(temp_pairs2(~i_doubles,:),1);
            SelElecs.Pairs.PE_Pred.Temporal_Frontal.Ind_UniquePairings_persub{i_sub} = ...
                temp_pairs2(~i_doubles,:);
        elseif size(temp_pairs1,1) == 1
            SelElecs.Pairs.PE_Pred.Temporal_Frontal.Num_UniquePairings_persub(1,i_sub) = ...
                1;
    ...
    set(gcf,'units','normalized','outerposition',[0 0 1 1]) %full screen
    DimSubplot = [3 3];
    sgtitle(['Possible Electrode Pairs - {\color{blue}PE} to {\color{red}Pred} (p < ' num2str(param.pval_plotting) ', ' FDR_label ') - {\color{blue}Temporal} - {\color{red}Frontal}'])
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}))
            if ~isnan(SelElecs.Pairs.PE_Pred.Temporal_Frontal.Ind_UniquePairings_persub{i_sub})
                curr_PEelec_ind = unique(SelElecs.Pairs.PE_Pred.Temporal_Frontal.Ind_UniquePairings_persub{i_sub}(:,1));
                curr_Predelec_ind = unique(SelElecs.Pairs.PE_Pred.Temporal_Frontal.Ind_UniquePairings_persub{i_sub}(:,2));
            else
                curr_PEelec_ind = find(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}));
                curr_Predelec_ind = find(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}));
            end
            ...
            elec_typeindex = [ones(length(curr_PEelec_ind),1); ones(length(curr_Predelec_ind),1)*2]; %1 = source, 2 = target
            elec_typeindex(filt_elecs_botheffects) = 4;
            elec_pairings = SelElecs.Pairs.PE_Pred.Temporal_Frontal.Ind_UniquePairings_persub{i_sub};
            effect_comp = 'PE_Pred';
            chanSize = ones(1,length(elec_data))*3; %electrode size (arbitrary)
            clims = [1 9];
            cmap  = ones(length(subs), 3).*[0.6207    0.3103    0.2759];
            ...
            sp_title = {[subs{i_sub} ' - ' num2str(length(curr_PEelec_ind)) ' PE Elecs / '  ...
                num2str(length(curr_Predelec_ind)) ' Pred elecs / ' num2str(num_elecs_botheffects) ' Pred+PE elecs - '...
                num2str(SelElecs.Pairs.PE_Pred.Temporal_Frontal.Num_UniquePairings_persub(i_sub)) ' elec pairs']};
    ...
    filename     = ['Surf1H_' ...
        'Allsubn' num2str(length(subs)) '_' ...
        'PairsSignElec_PE2Pred_Front2Temp_'  ...
        'AllTD_' folderlabel_inputsignal '.png'];
    figfile      = [path_fig filename];
    saveas(gcf, figfile, 'png'); %save png version
    close;
