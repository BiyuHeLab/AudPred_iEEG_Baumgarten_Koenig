...
param.GC.tone_aggregation1 = {1, 6, 11, 16, 21, 26, 31};
param.GC.tone_aggregation2 = {2, 7, 12, 17, 22, 27, 32};

param.GC.tone_aggregation_label1 = cellfun(@(c)[c],param.GC.tone_aggregation1);
param.GC.tone_aggregation_label1 = num2str((param.GC.tone_aggregation_label1),'_%d');
param.GC.tone_aggregation_label1 = erase(param.GC.tone_aggregation_label1, ' ');

param.GC.tone_aggregation_label2 = cellfun(@(c)[c],param.GC.tone_aggregation2);
param.GC.tone_aggregation_label2 = num2str((param.GC.tone_aggregation_label2),'_%d');
param.GC.tone_aggregation_label2 = erase(param.GC.tone_aggregation_label2, ' ');
...
% Load data for 1...6...11... etc
path_inputdata = ([paths_NASTD_ECoG.ECoGdata_Connectivity 'GC/']);
label_inputdata1 = ['GCdata_n9_' ...
    param.GC.ElecPairRegion '_' ...
    param.GC.InputDataType{1} param.GC.tone_aggregation_label1 '_' ...
    param.GC.epochdur_ms_label '_ensemnorm'];
GCdata1 = load([path_inputdata label_inputdata1], 'GCdata');
GCdata1 = GCdata1.GCdata;

% Load data for 2...7...12... etc
label_inputdata2 = ['GCdata_n9_' ...
    param.GC.ElecPairRegion '_' ...
    param.GC.InputDataType{1} param.GC.tone_aggregation_label2 '_' ...
    param.GC.epochdur_ms_label '_ensemnorm'];
GCdata2 = load([path_inputdata label_inputdata2], 'GCdata');
GCdata2 = GCdata2.GCdata;

%% 2) Determine and read out anatomical regions
for i_ToneSeqType = 1:2
    if i_ToneSeqType == 1
        GCdata = GCdata1;
    else
        GCdata = GCdata2;
...
%         end
    ...
    if i_ToneSeqType == 1
        Gavg_tempGC_peranatreg1 = Gavg_tempGC_peranatreg;
    else
        Gavg_tempGC_peranatreg2 = Gavg_tempGC_peranatreg;
    end
end

%% Compute the number of electrodes per frontal, temporal, parietal lobes for prediction and PE -- UPDATE: THIS DOESN'T WORK YET
% Initialize counters for Frontal, Parietal, and Temporal regions
NumElecs_perregion_pereffect = zeros(3,2); % Frontal, Parietal, Temporal x Prediction, PE

% Define keywords to identify the regions
frontalKeywords = {'AntPFC', 'AntPFC_IFG', 'AntPFC_PrecentralG','AntPFC_PrecentralG_SupParLob_PostcentralG','Frontal','IFG','PrecG'}; 
parietalKeywords = {'SupParLob_SupramarginalG', 'SupParLob', 'SupParLob_PostcentralG', 'SupParLob_PostcentralG_SupramarginalG','SupmarG','Parietal','PostcG'}; 
temporalKeywords = {'VentralT_STG', 'VentralT_MTG', 'VentralT_STG_MTG', 'VentralT','STG', 'MTG','Temporal'};
...
for i_effect = [1 3]
    ...
    countTemporal = 0;
    for i_sub = 1:9
        % Get the regions for the current subject
        regions = anatreg{i_effect}.sourceelec_label{i_sub};

        % Loop through each region
        for i = 1:length(regions)
            region = regions{i}; % Get the current region name
            classified = false; % Flag to track if the region is classified

            % Check if the region belongs to Frontal lobe
            if any(cellfun(@(x) contains(region, x, 'IgnoreCase', true), frontalKeywords))
                countFrontal = countFrontal + 1;
                classified = true;
            end


            % Check if the region belongs to Parietal lobe
            if any(cellfun(@(x) contains(region, x, 'IgnoreCase', true), parietalKeywords))
                countParietal = countParietal + 1;
                classified = true;
            end

            % Check if the region belongs to Temporal lobe
            if any(contains(region, temporalKeywords, 'IgnoreCase', true))
                countTemporal = countTemporal + 1;
                classified = true;
            end

            if ~classified
                fprintf('Unclassified region: %s\n', region);
            end
        end
    end
    if i_effect == 1
        NumElecs_perregion_pereffect(:,1) = [countFrontal, countParietal, countTemporal];
    else
        NumElecs_perregion_pereffect(:,2) = [countFrontal, countParietal, countTemporal];
    end
end
...
% durations:
Gavg_tempGC_peranatreg_avg = Gavg_tempGC_peranatreg1;  % Copy the structure to preserve its shape

% To plot for each tone sequence separately
ToneSeq1_GC = Gavg_tempGC_peranatreg1;
ToneSeq2_GC = Gavg_tempGC_peranatreg1;
...
for i = 1:4
    ...
    avg_cell1 = cell(10,10); %to plot GC across tone sequence 1
    avg_cell2 = cell(10,10); %to plot GC across tone sequence 2

    for j = 1:10
        for k = 1:10
            % Extract the relevant data from both Gavg_tempGC_peranatreg1 and Gavg_tempGC_peranatreg2
            data1_1 = Gavg_tempGC_peranatreg1.source2target{i}{j, k, 1}; %data for tone sequence 1, with tone duration 1
            data1_2 = Gavg_tempGC_peranatreg1.source2target{i}{j, k, 2}; %data for tone sequence 1, with tone duration 2
            data2_1 = Gavg_tempGC_peranatreg2.source2target{i}{j, k, 1}; %data for tone sequence 2, with tone duration 1
            data2_2 = Gavg_tempGC_peranatreg2.source2target{i}{j, k, 2}; %data for tone sequence 2, with tone duration 2
            
            % Average the data across both structures and both time points (1 and 2)
            avg_data = (data1_1 + data1_2 + data2_1 + data2_2) / 4;
            avg_data1 = (data1_1 + data1_2) / 2;
            avg_data2 = (data2_1 + data2_2) / 2;
            
            % Store the result in the averaged_cell array
            averaged_cell{j, k} = avg_data;
            avg_cell1{j,k} = avg_data1;
            avg_cell2{j,k} = avg_data2;
        end
    end

    % Store the averaged data back into the output structure
    Gavg_tempGC_peranatreg_avg.source2target{i} = averaged_cell;
    ToneSeq1_GC.source2target{i} = avg_cell1;
    ToneSeq2_GC.source2target{i} = avg_cell2;
end
Gavg_tempGC_peranatreg_avg = rmfield(Gavg_tempGC_peranatreg_avg, {'target2source', 'subject_index'});
ToneSeq1_GC = rmfield(ToneSeq1_GC, {'target2source', 'subject_index'});
ToneSeq2_GC = rmfield(ToneSeq2_GC, {'target2source', 'subject_index'});
...
index_pairs = { ...
    [1, 2], ... %Frontal to Temporal
    [2, 1], ... %Temporal to Frontal
    [1, 3], ... %Frontal to Parietal
    [3, 1], ... %Parietal to Frontal
    [2, 3], ... % Temporal to Parietal
    [3, 2] ... %Parietal to Temporal
};
...
for i_effect = 1:4 % Pred-Pred or PE-PE
    temp_currfilt = find(anatreg{i_effect}.filter == 1);
    for pair_idx = 1:(length(index_pairs))
        ...
        hold(f_sub1, 'on');
        plot(f_sub1, (1:length(param.GC.tone_aggregation1)) - 0.05, ...
        ...
        f_sub1.XLim = [1 - 0.3, length(param.GC.tone_aggregation1) + 0.3];
        ...
        [~,pval,~,tstats] = ttest(coeff_linfit(:,1),0,'tail','both');
        if pval < 0.001
            pval_text = 'p < 0.001';
        else
            pval_text = ['p = ' num2str(round(pval, 3))];
        end
        text(1.3, f_sub1.YLim(2)*0.75, ...
            {'Slopes LinMod vs. 0', ...
            ['avg slope = ' char(vpa(round(nanmean(coeff_linfit(:,1)),6))) '; ' pval_text]}, ...
            'Color',[0 0 1])
        ...
        savePath = [figures_dir, 'GC_1to31_', param.GC.ElecPairEffect{i_effect}, '_', anatreg{i_effect}.label_aggregated{temp_currfilt(temp_currfilt_pair(1))}, anatreg{i_effect}.label_aggregated{temp_currfilt(temp_currfilt_pair(2))}, '_withTrendline_pval.eps'];
