function NASTD_ECoG_Predict_CompPred_TW_Subs_FTPL...
...
trialID_p34 = FuncInput_InputData.trialinfo(:,1);
trialID_FTPL = table2array(FTPLratings(:,3));
[~, idx_match] = ismember(trialID_p34, trialID_FTPL);
...
for i_k = 1:length(param.PredSeqrange)
    for i_win = 1:size(windows,1)
        for i_sensor = 1:nSensors
            ...
            %6.1 Prediction Error (FTLP) Effect modulating p34
            dv = ERF_p34_win{i_win}(i_sensor,:)';  %p34 ERF (averaged across window) at this sensor for all trials 
            iv_pred = FTPL_pertrial; %FTPL ratings for all trials          
            % linear regression between ERF (per channel, time window) and predicted tone pitch
            stats = regstats(dv, iv_pred, 'linear', 'tstat');            
            PredErrFTPL.stats{i_k}{i_win}{i_sensor}  = stats;
            PredErrFTPL.tval{i_k}{i_win}(i_sensor)   = stats.tstat.t(2);
            PredErrFTPL.pval{i_k}{i_win}(i_sensor)   = stats.tstat.pval(2);
            PredErrFTPL.beta{i_k}{i_win}(i_sensor)   = stats.tstat.beta(2);
            stats = [];
