function NASTD_ECoG_Predict_PlotERFs...
    (sub, FuncInput_DataType, FuncInput_ToneDur_text,  ...
    param,...
    FuncInput_InputData, ...
    save_poststepFigs, paths_NASTD_ECoG, signElecIdx)
...
%ERF for p33
nSensors = length(signElecIdx);
NumFig = ceil(nSensors/30);
...
for i_fig = 1:NumFig
    ...
    for i_subplot = 1:30
        if i_elec_counter < nSensors

            i_elec_counter = i_elec_counter + 1;
            i_elec = signElecIdx(i_elec_counter);  % <-- KEY CHANG
