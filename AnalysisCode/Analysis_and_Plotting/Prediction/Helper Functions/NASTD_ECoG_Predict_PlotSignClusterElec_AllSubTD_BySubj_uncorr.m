function NASTD_ECoG_Predict_PlotSignClusterElec_AllSubTD_BySubj_uncorr...
    (subs, ...
...
SignElecs.array         = any(Param2plot.all_subs.pval_derivative > 1.3,2); %1D filter denoting sign. elecs (independent of number of clusters)
...
dat.avg         = subjID_sigelecs;
Sig_elecs_forcolor = any(Param2plot.all_subs.pval_FDR < param.pval_plotting,2);
dat.sign_elecs  = Sig_elecs_forcolor;
...
PlotInput_forcolor = subjID_sigelecs;

% Set large size for FDR-significant electrodes
chanSize(Sig_elecs_forcolor) = SizeFactor_FDR;
...
sp_handle_surf_temp = NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH(...
    coords_plot, labels_plotting_number, PlotInput_forcolor, ~Sig_elecs_forcolor,...
...
if any(Sig_elecs_forcolor)
    [sphereX, sphereY, sphereZ] = sphere(20);
    for i = find(Sig_elecs_forcolor)'
        cx = coords_plot(i,1);
        cy = coords_plot(i,2);
        cz = coords_plot(i,3) + 5;  % Stronger z-shift
...
if save_poststepFigs == 1
    filename     = ['Surf1HSignElec' param.ElecSelect ...
        ...
        '_AllTD_Stat' FDR_label '_withUncorr.png'];
