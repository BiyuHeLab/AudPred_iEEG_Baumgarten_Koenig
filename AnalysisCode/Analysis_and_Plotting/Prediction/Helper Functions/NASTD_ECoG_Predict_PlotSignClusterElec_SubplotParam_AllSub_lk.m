function NASTD_ECoG_Predict_PlotSignClusterElec_SubplotParam_AllSub_lk...
    (subs, ...
...
for i_sub = 1:length(subs)
    ...
    load([path_inputdata ...
        ...
        FuncInput_ToneDur_text 'sTD_NoClusterCorr.mat'], ...
    ...
    nSamples    = length(CurrentEffect.pval_FDR(1,:));
    ...
    Param2plot.per_sub.pval{i_sub}                  = nan(nSensors_sel,1);
    Param2plot.per_sub.pval_FDR{i_sub}              = nan(nSensors_sel,1);
    Param2plot.per_sub.effect_onset{i_sub}          = nan(nSensors_sel,1);
    ...
    for i_elec = 1:nSensors_sel
        ...
        if ~isempty(sig_timepoints) %only take the information for the first significant timepoint
            ...
            Param2plot.per_sub.pval{i_sub}(i_elec,1) = CurrentEffect.pval{ind_signelecs_StimCorr(i_elec)}(1,i_t);
            Param2plot.per_sub.pval_FDR{i_sub}(i_elec,1) = CurrentEffect.pval_FDR{ind_signelecs_StimCorr(i_elec)}(1,i_t);
            Param2plot.per_sub.effect_onset{i_sub}(i_elec,1) = i_t / nSamples;
        end
    end
