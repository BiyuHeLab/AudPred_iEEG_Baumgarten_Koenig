function NASTD_Predict_PlotPrediction_Subs(sub, baseline_correct, tonedur_text, predictive_sequencerange, toneIndex, pval_plotting, save_poststepFigs)
...
association = {'pred_t1', 'pred_err_t1', 'simplepred_err_t1'};

for i = 3%1:length(association) %loop across measures of association
    ...
    %                     title_text = ['evaluated at tone #' num2str(toneIndex+1) ', prediction using k = ' num2str(ks(i_k)) ' tones'];                  
                    ...
                    case 'simplepred_err_t1'
                        association_title = 'abs(simple prediction error) linear regression t-stat';
                        pval = simplepred_err_t1_p{i_k}{i_win};
                        title_text = ['evaluated at tone #' num2str(toneIndex+1) ', prediction using k = ' num2str(predictive_sequencerange(i_k)) ' tones'];
                end
        ...
        %         dat.label  = data_ECoGfiltref_trials.label(1:subs_PreProcSettings.(sub).number_ECoGchan); %all ECoG chan
                ...
                hold on
                subFigtitle = {[sub ' - ' association_title] ['ToneDur: ' tonedur_text 'ms - ERP ' win_title] [title_text ', p < ' num2str(pval_plotting)]};
                ...
                if save_poststepFigs == 1
                    path_fig1 = [path_fig '2DLineplot/ERP/'];
                    mkdir([path_fig1]);
                    filename     = [sub '_' association{i} '_ERP_' w1 '-' w2 'msTW_' tonedur_text 'msTD.png'];
        ...
        %         coords = data_ECoGfiltref_trials.elec.chanpos(1:subs_PreProcSettings.(sub).number_ECoGchan,;); %all ECoG chan
                ...
                chanSize = (vals./vals)*3.5; %electrode size (arbitrary)
...
%                 clims = [-(max([dv_absmax, 1.1])), max([dv_absmax, 1.1])]; %free scaling
                ...
                Figtitle =  {[sub ' - ' association_title] ['ToneDur: ' tonedur_text 'ms - ERP ' win_title] [title_text ', p < ' num2str(pval_plotting)]};
                title(Figtitle)

                if save_poststepFigs == 1
                    path_fig2 = [path_fig '3DSurfplot/ERP/'];
                    mkdir([path_fig2]);
                    filename     = [sub '_' association{i} '_ERP_' w1 '-' w2 'msTW_' tonedur_text 'msTD_LH.png'];
                ...
                if strcmp(sub,'NY723') %For subject 4 also plot RH
                    ...
                    if save_poststepFigs == 1
                        path_fig2 = [path_fig '3DSurfplot/ERP/'];
                        mkdir([path_fig2]);
                        filename     = [sub '_' association{i} '_ERP_' w1 '-' w2 'msTW_' tonedur_text 'msTD_RH.png'];
                        figfile      = [path_fig2 filename];                
