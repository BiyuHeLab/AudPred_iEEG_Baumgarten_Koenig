...
%         %TW data
%         load([path_inputdata param.Label_TW 'sTW/' sub '_PredEffects_' FuncInput_DataType{i_inputData} '_' FuncInput_ToneDur_text{i_inputData} 'sTD.mat'], ...
...
%         
        ...
        %Sample-wise data
        load([path_inputdata 'Samplewise/' sub '_PredEffectsCluster_' FuncInput_DataType{i_inputData} '_' FuncInput_ToneDur_text{i_inputData} 'sTD.mat'], ...
            'PredEffect', 'SimplePredErrEffect', 'ComplexPredErrEffect', 'labels_loadedData');
        ...
        for i_effect = 1:length(FuncInput_EffectType)
            ...
            if ~isempty(tempSignElecIndices.uncorr)
                display([num2str(length(tempSignElecIndices.uncorr)) ...
                    ...
                    '; InputData: ' FuncInput_DataType{i_inputData} ...
                    '; Effect: ' FuncInput_EffectType{i_effect} ...
                    '; ToneDur: ' FuncInput_ToneDur_text{i_inputData}])
            end
            
            %2.3 Store in common var
            SignElecIndices.Cluster.uncorr{i_sub}.(...
                FuncInput_DataType{i_inputData}).(...
                FuncInput_EffectType{i_effect}).Index{i_inputData} = ...
                tempSignElecIndices.uncorr;
                ...
                if ~isempty(tempSignElecIndices.uncorr(i_elec))
                    for i_cluster = 1:(length(unique(temp_StatOutput{tempSignElecIndices.uncorr(i_elec)}.cluster_timecourse))-1)
                        if temp_StatOutput{tempSignElecIndices.uncorr(i_elec)}.cluster_pval(i_cluster) < param.alpha
                            ...
                            SignElecIndices.Cluster.List.uncorr{i_inputData,i_effect,i_inputData} = ...
                                [SignElecIndices.Cluster.List.uncorr{i_inputData,i_effect,i_inputData}; ...
                                sub FuncInput_DataType{i_inputData} FuncInput_EffectType{i_effect} FuncInput_ToneDur_text{i_inputData} ...
                                i_cluster ...
...
for i_inputData = 1:length(FuncInput_DataType)
    for i_effect = 1:length(FuncInput_EffectType)
        ...
        %                     'VariableNames',colNames);
        %             end
        if ~isempty(SignElecIndices.Cluster.List.uncorr{i_inputData,i_effect,i_inputData})
            SignElecIndices.Cluster.List.uncorr{i_inputData,i_effect,i_inputData} = ...
                array2table(SignElecIndices.Cluster.List.uncorr{i_inputData,i_effect,i_inputData},...
                'VariableNames',colNames);
        end
...
savefile = [path_dataoutput 'N' num2str(length(validSubjs)) '_SummaryPredEffects_RR.mat'];
