function Onset_times_ms = NASTD_ECoG_Predict_PlotSignClusterElec_SubplotParam_AllSubTD_FDR_lk...
    (subs, ...
...
%         Commented out by LK on Nov 20, 2024 and changed to the code below
    SignElecs.onset_mincluster(i_elec,1) = ...
        min(Param2plot.all_subs.effect_onset(SignElecs.index(i_elec), :));
...
%     end
%     
% %     %Optional: Make FDR-corrected sign. electrodes larger
% %     SignElecs_FDR = any(Param2plot.all_subs.pval_FDR < 0.025,2); 
% %  	chanSize    = ones(1,length(dat.avg))*SizeFactor; %electrode size (arbitrary)
% %     chanSize(SignElecs_FDR) = SizeFactor*2;
%     
%     %Colorlim
...
% %     ColorbarPosition    = [0.875 0.025 0.035 0.95]; %Left of surface
% 
%     h = colorbar;
%     h.Position(1)       = ColorbarPosition(1); %sets colorbar to the right
%     h.Position(2)       = ColorbarPosition(2); %sets colorbar higher
...
%     CounterSubplot = CounterSubplot + 1;
    ...
    Plot_input_TD1 = nan(length(Param2plot.all_subs.effect_onset),1);
    Plot_input_TD1(Index_TD1) = OnsetTimes_TD1;
    Plot_input_TD2 = nan(length(Param2plot.all_subs.effect_onset),1);
    Plot_input_TD2(Index_TD2) = OnsetTimes_TD2;
    
%     for i_elec = 1:length(Param2plot.all_subs.effect_onset)
%         PlotInput(i_elec,1) = min(Param2plot.all_subs.effect_onset(i_elec,:));
%     end
%     mean(Param2plot.all_subs.effect_onset(SignElecs.array))
%     std(Param2plot.all_subs.effect_onset(SignElecs.array))
%     min(Param2plot.all_subs.effect_onset(SignElecs.array))
%     max(Param2plot.all_subs.effect_onset(SignElecs.array))
    

    %% TD1
    clims           = [0 200];
    Label_Colorbar  = {'effect onset in ms [rel. to tone]'};
    dat.avg         = Plot_input_TD1;
    
    % Plot it once for TD1
    sp_handle_surf_temp = NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH...
        (coords_allsub, labels_plotting_empty, dat.avg, SignElecs_TD1, ... % SignElecs.array,...
        chanSize, clims, cmap, dat.textcolor_rgb, ...
        DimSubplot, CounterSubplot, SubplotPosition, [], []);
    sp_handle_surf{CounterSurfplot} = sp_handle_surf_temp.L;
    CounterSubplot                  = CounterSubplot + 1;
    ...
    h1 = colorbar;
    h1.Position(1)       = ColorbarPosition(1); %sets colorbar to the right
    h1.Position(2)       = ColorbarPosition(2); %sets colorbar higher
    h1.Position(4)       = h.Position(4)*ColorbarPosition(4); %makes colorbar shorter
    h1.Label.String      = Label_Colorbar;
    h1.FontSize          = 16;
    h1.Ticks             = [0 0.25 0.5 0.75 1] * 200;
    h1.TickLength        = 0.1;
    caxis(clims)
    annotation(h, 'textbox', [0 0.75 0.5 0.05], 'String', 'Tone duration 200 ms', 'FontSize', 16, 'FontWeight', 'bold', 'EdgeColor', 'none', 'HorizontalAlignment', 'center');
    
    %% TD2
    clims           = [0 400];
    Label_Colorbar  = {'effect onset in ms [rel. to tone]'};
    dat.avg         = Plot_input_TD2;
    
    % Plot it again for TD2
    sp_handle_surf_temp = NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_LH...
        (coords_allsub, labels_plotting_empty, dat.avg, SignElecs_TD2, ... % SignElecs.array,...
        chanSize, clims, cmap, dat.textcolor_rgb, ...
        DimSubplot, CounterSubplot, SubplotPosition, [], []);
    sp_handle_surf{CounterSurfplot} = sp_handle_surf_temp.L;
    CounterSubplot                  = CounterSubplot + 1;
    
    ColorbarPosition    = [0.53 0.2 0 0.5]; % Right of surface for TD2    h = colorbar;
    h2 = colorbar;
    h2.Position(1)       = ColorbarPosition(1); %sets colorbar to the right
    h2.Position(2)       = ColorbarPosition(2); %sets colorbar higher
    h2.Position(4)       = h.Position(4)*ColorbarPosition(4); %makes colorbar shorter
    h2.Label.String      = Label_Colorbar;
    h2.FontSize          = 16;
    h2.Ticks             = [0 0.25 0.5 0.75 1] * 400;
    h2.TickLength        = 0.1;
    caxis(clims)
    annotation(h, 'textbox', [0.5 0.75 0.5 0.05], 'String', 'Tone duration 400 ms', 'FontSize', 16, 'FontWeight', 'bold', 'EdgeColor', 'none', 'HorizontalAlignment', 'center');
