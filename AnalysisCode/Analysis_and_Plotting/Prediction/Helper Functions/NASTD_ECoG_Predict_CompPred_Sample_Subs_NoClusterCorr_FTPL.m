function NASTD_ECoG_Predict_CompPred_Sample_Subs_NoClusterCorr_FTPL...
...
NumEffects.ElecsWithFDRSig.PredErrFTPL = 0;
...
for i_elec = 1:nSensors
    ...
    if sum(PredErrFTPL.pval_FDR{i_elec} < 0.05) > 0
        NumEffects.ElecsWithFDRSig.PredErrFTPL = NumEffects.ElecsWithFDRSig.PredErrFTPL + 1;
    end
end

disp(['For ' FuncInput_DataType ' & ' FuncInput_ToneDur_text 'TD, ']);
disp([num2str(NumEffects.ElecsWithFDRSig.PredErrFTPL) '/' num2str(nSensors) ...
...
savefile = [path_dataoutput sub '_PredEffectsFTPL_' FuncInput_DataType '_' FuncInput_ToneDur_text 'sTD_NoClusterCorr.mat'];
