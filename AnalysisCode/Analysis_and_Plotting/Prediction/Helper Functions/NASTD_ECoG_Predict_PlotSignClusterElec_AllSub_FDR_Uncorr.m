function NASTD_ECoG_Predict_PlotSignClusterElec_AllSub_FDR_Uncorr...
    (subs, ...
...
%     end    
    ...
    if param.FDRcorrect == 1
        ...
        Param2plot.per_sub.pval_FDR{i_sub} = cluster_pvalFDR_perelec;
    else
        FDR_label = 'uncorr';        
    end
    ...
    for i_elec = 1:length(Param2plot.per_sub.pval_FDR{i_sub})
        ...
        for i_cluster = 1:length(Param2plot.per_sub.pval_FDR{i_sub}(i_elec,:))
            %If a cluster is sign, enter its timecourse in the blank proxy
            %nd denote it with the cluster index
            if Param2plot.per_sub.pval_FDR{i_sub}(i_elec,i_cluster) < param.pval_plotting 
    ...
    if i_sub == 1
        ...
        Param2plot.all_subs.pval_FDR  = ...
            Param2plot.per_sub.pval_FDR{i_sub};
...
%% 3) Prepare & Plot Figure 1: FdrCorr and p-value derivative
%3.1 Prepare figure
...
SizeFactor_FDR = 5;     % large spheres for FDR significant
...
if any(SignElecs.array)
    [sphereX, sphereY, sphereZ] = sphere(10); % sphere mesh for plotting
    for i = find(SignElecs.array)'
        cx = coords_plot(i,1);
        cy = coords_plot(i,2);
        cz = coords_plot(i,3);
        radius = SizeFactor_FDR;
        surf(radius*sphereX + cx, radius*sphereY + cy, radius*sphereZ + cz, ...
            'FaceColor', 'yellow', 'EdgeColor', 'none', 'FaceAlpha', 1);
    end
end
...
textbox_info1 = {...
    ...
    [num2str(length(PlotInput(~isnan(PlotInput)))) ' clusters total'] []};
textbox_info2 = {};
...
if save_poststepFigs == 1
    filename     = ['Surf1HSignElec' param.ElecSelect ...
        ...
        '_' FuncInput_ToneDur_text 'msTD_Stat' FDR_label '_withUncorr.png'];
