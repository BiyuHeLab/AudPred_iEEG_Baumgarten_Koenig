function SignElecIndices = NASTD_ECoG_Predict_PredEffectTable_FTPL_lk...
    (validSubjs, sub_list, ...
...
for i_sub = 1:length(validSubjs)
    for i_inputData = 1:length(FuncInput_DataType)
        ...
        for i_TD = 1:length(FuncInput_ToneDur_text)
            ...
            %TW data
%             load([path_inputdata param.Label_TW 'sTW/' sub '_PredEffectsFTPL_' FuncInput_DataType{i_inputData} '_' FuncInput_ToneDur_text{i_TD} 'sTD.mat'], ...
%                 'PredErrFTPL', 'labels_loadedData');
%             
...
%             end
%             %Clean up
%             clear PredErrFTPL labels_loadedData
            
            
            %% 2. Sample-wise data
            %2.1 Load in sample-wise prediction effects and electrode labels
            %Sample-wise data
            load([path_inputdata 'Samplewise/' sub '_PredEffectsFTPL_' FuncInput_DataType{i_inputData} '_' FuncInput_ToneDur_text{i_TD} 'sTD.mat'], ...
                'PredErrFTPL', 'labels_loadedData');
