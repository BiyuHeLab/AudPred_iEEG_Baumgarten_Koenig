function sp_handle_surf = NASTD_ECoG_Plot_SubplotSignElecsSurf_Label_SubplotHemis...
...
if any(sign_index) %if there are any sign. channels
    ...
    for cn = 1:numChans
        ...
        u = text(coords_signElecs(cn,1)-50,coords_signElecs(cn,2)-10,coords_signElecs(cn,3),label_signElecs(cn),'Interpreter', 'none');
%         set(u,'Color',[0.8500, 0.3250, 0.0980],'FontSize',10,'FontWeight', 'bold'); %orange
%         set(u,'Color',[0,0.15,0],'FontSize',10,'FontWeight', 'bold'); %dark green
         set(u,'Color',[0,0,0],'FontSize',8,'FontWeight', 'bold'); %black
...
title([win_title ' Left'],'FontSize',8);

%% RH
sp_handle_surf.R = subplot(DimSubplot(1),DimSubplot(2),CounterSubplot+1);
pos2 = get(sp_handle_surf.R,'Position');
% newpos2 = pos2 + [0 0 0 0.04];
% newpos2 = pos2 + [0 0 0 DimSubplot(2)*0];
newpos2 = pos2 + SubplotPosition;
set(sp_handle_surf.R,'Position',newpos2)
...
if any(sign_index) %if there are any sign. channels
    ...
    for cn = 1:numChans
        ...
        %Add electrode labels add respective position        
        u = text(coords_signElecs(cn,1)+50,coords_signElecs(cn,2)-10,coords_signElecs(cn,3),label_signElecs(cn),'Interpreter', 'none');
        %+50 on x-axis to ensure that text is in front
%         set(u,'Color',[0.8500, 0.3250, 0.0980],'FontSize',10,'FontWeight', 'bold'); %orange
%         set(u,'Color',[0,0.15,0],'FontSize',10,'FontWeight', 'bold'); %dark green
         set(u,'Color',[0,0,0],'FontSize',8,'FontWeight', 'bold'); %black
    end
