function c = NASTD_ECoG_Plot_SubplotElecsSurf_SubplotHemis...
...
%% LH
sp_handle1 = subplot(DimSubplot(1), DimSubplot(2), CounterSubplot, 'align');
...
%     h.Position(4) = h.Position(4)*1.5; %makes colorbar shorter
    ylabel(h, 'Exp Kprime (uncorrected)')
    yticklabels('auto');
    % set(gca,'FontSize',16);
else
    colorbar off
...
%% RH
% sp_handle2 = subplot(DimSubplot(1)*2,DimSubplot(2),CounterSubplot+1, 'align');
sp_handle2 = subplot(DimSubplot(1),DimSubplot(2),CounterSubplot+1, 'align');
pos2 = get(sp_handle2,'Position');
pos2(2) = pos1(2);
set(sp_handle2,'Position',pos2);
