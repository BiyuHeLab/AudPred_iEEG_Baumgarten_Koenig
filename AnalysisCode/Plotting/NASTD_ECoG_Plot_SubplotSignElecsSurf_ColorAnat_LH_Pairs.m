function sp_handle_surf = NASTD_ECoG_Plot_SubplotSignElecsSurf_ColorAnat_LH_Pairs...
    (coords, SelElecLabes, vals, sign_index, ...
    pairings, effect_comp, ...
...
% pairings = elec_pairings_ordered
...
for cn = 1:numChans
    ...
    if numChans > (length(coords)/2)
        x = x - (abs(sign_vals(cn)*20)); %Places extreme values in front of plot
    ...
    for i_sourceelec = unique(pairings(:,1))'
        ...
        if length(connected_elecs) > 0
            for i_connected_elecs = 1:length(connected_elecs)
                ...
                hold on;
                l = line([(x-50) (x_other-50)],[y y_other],[z z_other], 'LineWidth', 0.5);
                stepsize_colormap = 1;
                if length(pairings) > 1000
                    l.Color = [0.3, 0.3, 0.3, 0.1]; %Color & Transparency
                elseif length(pairings) > 100
