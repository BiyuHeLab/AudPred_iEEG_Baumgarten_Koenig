function stim_corr_anova_topo(sub, DV_ind, saveplot, phase_option, input_betaID)
...
path_base  = '/data/gogodisk2/brian/';
...
path_in  = [path_base 'analysis/sfa_expt2_v2/stimulus_tracking/stim_corr_by_beta/stim_corr_results/' sub '/'];
...
%         figfile  = [sub '_stim_corr_ANOVA_within_' DV '_unwrap_subtr_exp.png'];     
    ...
    otherwise
        filename = [sub '_stim_corr_' DV '_betaID=' num2str(input_betaID) '.mat'];
        figfile  = [sub '_stim_corr_ANOVA_within_' DV '_betaID=' num2str(input_betaID) '.png'];    
...
mkdir([path_base 'analysis/sfa_expt2_v2/stimulus_tracking/stim_corr_by_beta/figures/'], sub);
path_fig    = [path_base 'analysis/sfa_expt2_v2/stimulus_tracking/stim_corr_by_beta/figures/' sub '/'];
% saveplot    = 0;
...
%     cfg.marker    = 'numbers';
    ...
    if plot_pval
        ...
        title({[sub ' expt 2 within vs across stim correlations'], ...
               ['using MEG DV = ' corr_within{input_betaID,1,1}.DV_label], ...
               ['ANOVA F values for ' factor_labels{i_factor} ' for stim beta = ' num2str(betas(input_betaID)) ' (plotting with Bonferroni correction, p < ' num2str(sig_p) ')']}, ...
                'FontSize', fontsize);
    end
