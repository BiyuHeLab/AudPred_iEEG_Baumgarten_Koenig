...
%Aim: Filter (LP 35Hz) data containing interspersed NaNs and interpolate
...
FilterOrder     = 3;
LowPassFreq     = 35;
[lpb,lpa]       = butter(FilterOrder,(LowPassFreq*2)/input_data.fsample,'low');
    ...
    for i_elec = 1:length(input_data.label)
        ...
        if isempty(ind_NaNsamples{i_trial}{i_elec})
            ...
            datastruct.trial{i_trial}(i_elec,:) = ...
                filtfilt(lpb,lpa,input_data.trial{i_trial}(i_elec,:)')';
        ...
        if ~isempty(index_nanelects)
            ...
            if save_poststepFigs == 1
                path_fig = [paths_NASTD_ECoG.Preproc_ECoGdata sub ...
                    '/Figs/Preproc/FiltInterp/LP35Hz/' ];
                if (~exist(path_fig, 'dir')); mkdir(path_fig); end

                filename     = strcat([sub '_FiltInterp_LP35Hz_TD' TD_label ...
