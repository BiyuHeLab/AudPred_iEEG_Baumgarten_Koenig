function NASTD_ECoG_Connectivity_PlotGC_persub_aggrtoneanat ...
    (subs, ...
...
NumGC_peranatreg = ...
    ...
    length(anatreg.label_aggregated), ...
    length(subs), ...
    length(ToneDur_text)); %Dim: AnatLabelSource, AnatLabelTarget, subs, TD
                        ...
                        for i_anatlabels_target = 1:length(anatreg.label_aggregated)
                            ...
                            for i_targetelec = 1:length(GCdata{i_sub}.ind_pairedelecs_to)
                                if any(anatreg.targetelec_index{i_sub}(i_targetelec) == anatreg.labelindex_aggregated(i_anatlabels_target,:))
                                    ...
                                    NumGC_peranatreg(i_anatlabels_source, i_anatlabels_target, i_sub, i_TD) = ...
                                        NumGC_peranatreg(i_anatlabels_source, i_anatlabels_target, i_sub, i_TD) + 1;
                                end
                            end
...
%     path_fig = ([paths_NASTD_ECoG.ECoGdata_Connectivity 'GC/Figs/persub/' sub '/' ...
%         title_label1 '_' title_label2 '/fullTW/']);
    path_fig = ([paths_NASTD_ECoG.ECoGdata_Connectivity 'GC/Figs/persub/' sub '/' ...
        title_label1 '_' title_label2 '/100mswin/']);
    ...
    for i_TD = 1:length(ToneDur_text)
        ...
        for i_tonegroup = 1:length(param.GC.tone_aggregation)
            maxC_tempGC{i_TD}(i_tonegroup,1) = max(max(nanmean(...
                Gavg_tempGC_peranatreg.source2target...
                (anatreg.filter,anatreg.filter,i_tonegroup,i_sub,i_TD))));
        end
        ...
        for i_tonegroup = 1:length(param.GC.tone_aggregation)
            ...
            NumGC_peranatreg_filter = NumGC_peranatreg(anatreg.filter, anatreg.filter, i_sub, i_TD);
            for i_imagefield_x = 1:size(Gavg_tempGC_peranatreg.source2target...
                    (anatreg.filter,anatreg.filter,i_tonegroup,i_sub,i_TD),1)
                for i_imagefield_y = 1:size(Gavg_tempGC_peranatreg.source2target...
                        (anatreg.filter,anatreg.filter,i_tonegroup,i_sub,i_TD),2)
        ...
        if ismember(toneindex_noreverse, cellfun(@(c)[c],param.GC.tone_aggregation))
            ...
            for i_tonegroup = index_noreverse
                ...
                colorbar
                title(['Time-domain (' sub ') GC estimates - Tone ' ...
        ...
        for i_sourceelec = 1:length(anatreg.label_aggregated(anatreg.filter))
            for i_targetelec = 1:length(anatreg.label_aggregated(anatreg.filter))
                ...
                Gavg_tempGC_peranatreg_filter.source2target = ...
                    Gavg_tempGC_peranatreg.source2target(anatreg.filter,anatreg.filter,:,i_sub,i_TD);
                ...
                NumGC_peranatreg_filter = ...
                    NumGC_peranatreg(anatreg.filter, anatreg.filter, i_sub, i_TD);
        ...
        for i_tonegroup = 1:length(param.GC.tone_aggregation)
            maxY_spectGC{i_TD}(i_tonegroup,1) = ...
                max(max(max(nanmean(...
                Gavg_spectGC_peranatreg.source2target...
                (anatreg.filter,anatreg.filter,temp_xlim(1):temp_xlim(2),i_tonegroup, i_sub, i_TD),4))));
        end
        ...
        for i_sourceelec = 1:length(anatreg.label_aggregated(anatreg.filter))
            for i_targetelec = 1:length(anatreg.label_aggregated(anatreg.filter))
                ...
                Gavg_spectGC_peranatreg_filter.source2target = ...
                    Gavg_spectGC_peranatreg.source2target(anatreg.filter,anatreg.filter,:,:,i_sub,i_TD);
        ...
        sgtitle(['Spectral (' sub ') pw GC estimates per tone group per anat region - ' ...
            ToneDur_text{i_TD} 's TD - ' param.GC.label_ElecPairSel{1} ', ' ...
            param.GC.label_ElecPairSel{2} ' - ' param.GC.InputDataType{1}], ...
            'Interpreter', 'none')
