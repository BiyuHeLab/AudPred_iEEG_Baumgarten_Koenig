function NASTD_ECoG_Connectivity_PlotSignElecCon_AllSubTDLobes...
    (subs, ...
...
if plot_poststepFigs == 1
    ...
    %Select electrodes and pairing for all lobe-wise connections
    curr_elec_ind_Frontal_Temporal = [];
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings_persub{i_sub})
                ...
                if iscolumn(unique(SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings_persub{i_sub}))
                    curr_elec_ind_Frontal_Temporal = [curr_elec_ind_Frontal_Temporal; ...
    ...
    elec_coords_Frontal_Temporal = SelElecs.PredEffect{curr_elec_ind_Frontal_Temporal,7};
    elec_coords_Frontal_Temporal(:,1) = abs(elec_coords_Frontal_Temporal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Frontal_Temporal = ones(size(elec_coords_Frontal_Temporal,1),1);
    elec_signindex_Frontal_Temporal = true(size(elec_coords_Frontal_Temporal,1),1);
    elec_typeindex_Frontal_Temporal = ones(size(elec_coords_Frontal_Temporal,1),1)*3;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Temporal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Temporal,5}, 'VentralT'))) = 2;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Temporal,5}, 'SupParLob'))) = 3;
    chanSize_Frontal_Temporal = ones(1,length(elec_data_Frontal_Temporal))*2; %electrode size (arbitrary)

    
    curr_elec_ind_Frontal_Parietal = [];
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_Pred.Frontal_Parietal.Ind_UniquePairings_persub{i_sub})
                ...
                if iscolumn(unique(SelElecs.Pairs.Pred_Pred.Frontal_Parietal.Ind_UniquePairings_persub{i_sub}))
                    curr_elec_ind_Frontal_Parietal = [curr_elec_ind_Frontal_Parietal; ...
    ...
    elec_coords_Frontal_Parietal = SelElecs.PredEffect{curr_elec_ind_Frontal_Parietal,7};
    elec_coords_Frontal_Parietal(:,1) = abs(elec_coords_Frontal_Parietal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Frontal_Parietal = ones(size(elec_coords_Frontal_Parietal,1),1);
    elec_signindex_Frontal_Parietal = true(size(elec_coords_Frontal_Parietal,1),1);
    elec_typeindex_Frontal_Parietal = ones(size(elec_coords_Frontal_Parietal,1),1)*3;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Parietal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Parietal,5}, 'VentralT'))) = 2;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Frontal_Parietal,5}, 'SupParLob'))) = 3;
    chanSize_Frontal_Parietal = ones(1,length(elec_data_Frontal_Parietal))*2; %electrode size (arbitrary)

    
    curr_elec_ind_Parietal_Temporal = [];
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PredEffect{:,2}))
            if ~isnan(SelElecs.Pairs.Pred_Pred.Parietal_Temporal.Ind_UniquePairings_persub{i_sub})
                ...
                if iscolumn(unique(SelElecs.Pairs.Pred_Pred.Parietal_Temporal.Ind_UniquePairings_persub{i_sub}))
                    curr_elec_ind_Parietal_Temporal = [curr_elec_ind_Parietal_Temporal; ...
    ...
    elec_coords_Parietal_Temporal = SelElecs.PredEffect{curr_elec_ind_Parietal_Temporal,7};
    elec_coords_Parietal_Temporal(:,1) = abs(elec_coords_Parietal_Temporal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Parietal_Temporal = ones(size(elec_coords_Parietal_Temporal,1),1);
    elec_signindex_Parietal_Temporal = true(size(elec_coords_Parietal_Temporal,1),1);
    elec_typeindex_Parietal_Temporal = ones(size(elec_coords_Parietal_Temporal,1),1)*3;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Parietal_Temporal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Parietal_Temporal,5}, 'VentralT'))) = 2;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PredEffect{curr_elec_ind_Parietal_Temporal,5}, 'SupParLob'))) = 3;
    chanSize_Parietal_Temporal = ones(1,length(elec_data_Parietal_Temporal))*2; %electrode size (arbitrary)
    ...
    elec_pairings_Frontal_Temporal = SelElecs.Pairs.Pred_Pred.Frontal_Temporal.Ind_UniquePairings;
    elec_pairings_Frontal_Temporal = elec_pairings_Frontal_Temporal(~isnan(elec_pairings_Frontal_Temporal(:,1)),:);
    elec_pairings_ordered_Frontal_Temporal = zeros(size(elec_pairings_Frontal_Temporal));
    for i_row = 1:size(elec_pairings_Frontal_Temporal,1)
        for i_column = 1:size(elec_pairings_Frontal_Temporal,2)
            elec_pairings_ordered_Frontal_Temporal(i_row,i_column) = ...
                find(curr_elec_ind_Frontal_Temporal == elec_pairings_Frontal_Temporal(i_row,i_column));
        end
    end    
    
    elec_pairings_Frontal_Parietal = SelElecs.Pairs.Pred_Pred.Frontal_Parietal.Ind_UniquePairings;
    elec_pairings_Frontal_Parietal = elec_pairings_Frontal_Parietal(~isnan(elec_pairings_Frontal_Parietal(:,1)),:);
    elec_pairings_ordered_Frontal_Parietal = zeros(size(elec_pairings_Frontal_Parietal));
    for i_row = 1:size(elec_pairings_Frontal_Parietal,1)
        for i_column = 1:size(elec_pairings_Frontal_Parietal,2)
            elec_pairings_ordered_Frontal_Parietal(i_row,i_column) = ...
                find(curr_elec_ind_Frontal_Parietal == elec_pairings_Frontal_Parietal(i_row,i_column));
        end
    end 
    
    elec_pairings_Parietal_Temporal = SelElecs.Pairs.Pred_Pred.Parietal_Temporal.Ind_UniquePairings; 
    elec_pairings_Parietal_Temporal = elec_pairings_Parietal_Temporal(~isnan(elec_pairings_Parietal_Temporal(:,1)),:);
    elec_pairings_ordered_Parietal_Temporal = zeros(size(elec_pairings_Parietal_Temporal));
    for i_row = 1:size(elec_pairings_Parietal_Temporal,1)
        for i_column = 1:size(elec_pairings_Parietal_Temporal,2)
            elec_pairings_ordered_Parietal_Temporal(i_row,i_column) = ...
                find(curr_elec_ind_Parietal_Temporal == elec_pairings_Parietal_Temporal(i_row,i_column));
        end
    end
        
    sp_title = {['{\color{blue}Frontal}, {\color{red}Temporal}, {\color{green}Parietal} {\color{black}electrodes}  '], ...
        ['Pred elecs']};
    
    
    Pairing2plot = 'Parietal_Temporal'; %Frontal_Temporal Frontal_Parietal Parietal_Temporal
    
    NASTD_ECoG_Plot_SubplotSignElecsSurf_ColorAnatAllLobes_LH_Pairs...
        (elec_coords_Frontal_Temporal, elec_coords_Frontal_Parietal, elec_coords_Parietal_Temporal,...
        elec_typeindex_Frontal_Temporal, elec_typeindex_Frontal_Parietal, elec_typeindex_Parietal_Temporal, ...
        elec_signindex_Frontal_Temporal, elec_signindex_Frontal_Parietal, elec_signindex_Parietal_Temporal, ...
        elec_pairings_ordered_Frontal_Temporal, elec_pairings_ordered_Frontal_Parietal, elec_pairings_ordered_Parietal_Temporal, ...
        Pairing2plot, ...
        chanSize_Frontal_Temporal, chanSize_Frontal_Parietal, chanSize_Parietal_Temporal, ...
        clims, cmap, ...
        [1 1], 1, [0 -0.02 0 0], sp_title);
    ...
    'AllTDLobes_' Pairing2plot '_' folderlabel_inputsignal '.png'];
...
if plot_poststepFigs == 1
    ...
    for i_sub = 1:length(subs)
        if any(strcmp(subs(i_sub),SelElecs.PEEffect{:,2}))
            if ~isnan(SelElecs.Pairs.PE_PE.Frontal_Temporal.Ind_UniquePairings_persub{i_sub})
                %Note: the indices refer to the respective effect-specific  across-subject electrode listing
                %(e.g., 5 = 5th electrode in SelElecs.PEEffect)
    ...
    elec_coords_Frontal_Temporal = SelElecs.PEEffect{curr_elec_ind_Frontal_Temporal,7};
    elec_coords_Frontal_Temporal(:,1) = abs(elec_coords_Frontal_Temporal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Frontal_Temporal = ones(size(elec_coords_Frontal_Temporal,1),1);
    elec_signindex_Frontal_Temporal = true(size(elec_coords_Frontal_Temporal,1),1);
    elec_typeindex_Frontal_Temporal = ones(size(elec_coords_Frontal_Temporal,1),1)*3;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Temporal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Temporal,5}, 'VentralT'))) = 2;
    elec_typeindex_Frontal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Temporal,5}, 'SupParLob'))) = 3;
    chanSize_Frontal_Temporal = ones(1,length(elec_data_Frontal_Temporal))*2; %electrode size (arbitrary)
    ...
    elec_coords_Frontal_Parietal = SelElecs.PEEffect{curr_elec_ind_Frontal_Parietal,7};
    elec_coords_Frontal_Parietal(:,1) = abs(elec_coords_Frontal_Parietal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Frontal_Parietal = ones(size(elec_coords_Frontal_Parietal,1),1);
    elec_signindex_Frontal_Parietal = true(size(elec_coords_Frontal_Parietal,1),1);
    elec_typeindex_Frontal_Parietal = ones(size(elec_coords_Frontal_Parietal,1),1)*3;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Parietal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Parietal,5}, 'VentralT'))) = 2;
    elec_typeindex_Frontal_Parietal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Frontal_Parietal,5}, 'SupParLob'))) = 3;
    chanSize_Frontal_Parietal = ones(1,length(elec_data_Frontal_Parietal))*2; %electrode size (arbitrary)
    ...
    elec_coords_Parietal_Temporal = SelElecs.PEEffect{curr_elec_ind_Parietal_Temporal,7};
    elec_coords_Parietal_Temporal(:,1) = abs(elec_coords_Parietal_Temporal(:,1)) * -1;%Project all electrodes on one hemisphere
    elec_data_Parietal_Temporal = ones(size(elec_coords_Parietal_Temporal,1),1);
    elec_signindex_Parietal_Temporal = true(size(elec_coords_Parietal_Temporal,1),1);
    elec_typeindex_Parietal_Temporal = ones(size(elec_coords_Parietal_Temporal,1),1)*3;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Parietal_Temporal,5}, 'AntPFC'))) = 1;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Parietal_Temporal,5}, 'VentralT'))) = 2;
    elec_typeindex_Parietal_Temporal(~cellfun('isempty',strfind(SelElecs.PEEffect{curr_elec_ind_Parietal_Temporal,5}, 'SupParLob'))) = 3;
    chanSize_Parietal_Temporal = ones(1,length(elec_data_Parietal_Temporal))*2; %electrode size (arbitrary)
    
    %Note: Problem: elec_coords is mixed (source + target) and ordered,
    %while elec_pairings is not mixed and not ordered. Both var refer to
    %the same elecs and thus should have the same orientation
    elec_pairings_Frontal_Temporal = SelElecs.Pairs.PE_PE.Frontal_Temporal.Ind_UniquePairings;
    elec_pairings_Frontal_Temporal = elec_pairings_Frontal_Temporal(~isnan(elec_pairings_Frontal_Temporal(:,1)),:);
    elec_pairings_ordered_Frontal_Temporal = zeros(size(elec_pairings_Frontal_Temporal));
    for i_row = 1:size(elec_pairings_Frontal_Temporal,1)
        for i_column = 1:size(elec_pairings_Frontal_Temporal,2)
    ...
    elec_pairings_Frontal_Parietal = SelElecs.Pairs.PE_PE.Frontal_Parietal.Ind_UniquePairings;
    elec_pairings_Frontal_Parietal = elec_pairings_Frontal_Parietal(~isnan(elec_pairings_Frontal_Parietal(:,1)),:);
    elec_pairings_ordered_Frontal_Parietal = zeros(size(elec_pairings_Frontal_Parietal));
    for i_row = 1:size(elec_pairings_Frontal_Parietal,1)
        for i_column = 1:size(elec_pairings_Frontal_Parietal,2)
    ...
    elec_pairings_Parietal_Temporal = SelElecs.Pairs.PE_PE.Parietal_Temporal.Ind_UniquePairings; 
    elec_pairings_Parietal_Temporal = elec_pairings_Parietal_Temporal(~isnan(elec_pairings_Parietal_Temporal(:,1)),:);
    elec_pairings_ordered_Parietal_Temporal = zeros(size(elec_pairings_Parietal_Temporal));
    for i_row = 1:size(elec_pairings_Parietal_Temporal,1)
        for i_column = 1:size(elec_pairings_Parietal_Temporal,2)
