...
%1.4) Copy original trial number within TD-selection to trialinfo subfield, 
%so that we can use it so select later sequence-info
for i_trial = 1:length(data_input.trial)
    data_input.trialinfo(i_trial,3) = i_trial;
end
...
%% Test: Additional manuel preprocessing for whole trial duration
%Check in summary mode for suspicious trials
cfg             = [];
cfg.channel     = Selected_Channels; %only selected channels
cfg.latency     = [0 data_input.time{1}(end)];

% cfg.metric      = 'zvalue';
% cfg.method      = 'summary';
% cfg.keepchannel = 'yes';
% ft_rejectvisual(cfg,data_input);
% 
% %then check suspects in detail
% cfg.method      = 'channel';
% cfg.keepchannel = 'no';
% data_input_clean      = ft_rejectvisual(cfg,data_input);

cfg.method      = 'trial';
data_input_clean      = ft_rejectvisual(cfg,data_input);

%Update behav and stim fields to new trial selection
TrialFilt_CleanTrials = ismember(data_input.trialinfo(:,3),data_input_clean.trialinfo(:,3));
%Stim data
dat_fields  = fieldnames(data_input_clean.stim);
for i_field = 1:length(dat_fields)
    if eval(['length(data_input_clean.stim.' dat_fields{i_field} ') == ' num2str(length(data_input.trial))]) %if missed trials are encoded in current field
        eval(['data_input_clean.stim.' dat_fields{i_field} ' = data_input_clean.stim.' dat_fields{i_field} '(TrialFilt_CleanTrials);']);
    end
end
%Behav data
dat_fields  = fieldnames(data_input_clean.behav);
for i_field = 1:length(dat_fields)
    if eval(['length(data_input_clean.behav.' dat_fields{i_field} ') == ' num2str(length(data_input.trial))]) %if missed trials are encoded in current field
        eval(['data_input_clean.behav.' dat_fields{i_field} ' = data_input_clean.behav.' dat_fields{i_field} '(TrialFilt_CleanTrials);']);
    end
end
%Behav.stim data
dat_fields  = fieldnames(data_input_clean.behav.stim);
for i_field = 1:length(dat_fields)
    if eval(['length(data_input_clean.behav.stim.' dat_fields{i_field} ') == ' num2str(length(data_input.trial))]) %if missed trials are encoded in current field
        %             disp(['Adjusting subfield: ' dat_fields{i_field}])
        eval(['data_input_clean.behav.stim.' dat_fields{i_field} ' = data_input_clean.behav.stim.' dat_fields{i_field} '(TrialFilt_CleanTrials);']);
    end
end
%Behav.timing data
dat_fields  = fieldnames(data_input_clean.behav.timing);
for i_field = 1:length(dat_fields)
    if eval(['length(data_input_clean.behav.timing.' dat_fields{i_field} ') == ' num2str(length(data_input.trial))]) %if missed trials are encoded in current field
        %             disp(['Adjusting subfield: ' dat_fields{i_field}])
        eval(['data_input_clean.behav.timing.' dat_fields{i_field} ' = data_input_clean.behav.timing.' dat_fields{i_field} '(TrialFilt_CleanTrials);']);
    end
end
    
data_input =  data_input_clean;
...
if ~strcmp(ToneDur_text,'all') %for specific TD
    ...
    for i_trial = 1:nTrials    
        ECoGdata_p33(:, :, i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(toneIndex,1) : Sample_Tone_StartStop(toneIndex,2));
        ECoGdata_p34(:, :, i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(toneIndex+1,1) : Sample_Tone_StartStop(toneIndex+1,2)); 
        
        ECoGdata_p1_33(:, :, i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(1,1) : Sample_Tone_StartStop(toneIndex,2));        
    end
    ...
    for i_trial = 1:nTrials   
        ...
        ECoGdata_p33(:, 1:length(Sample_Tone_StartStop(toneIndex,1,i_trial) : Sample_Tone_StartStop(toneIndex,2,i_trial)), i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(toneIndex,1,i_trial) : Sample_Tone_StartStop(toneIndex,2,i_trial));
        ECoGdata_p34(:, 1:length(Sample_Tone_StartStop(toneIndex,1,i_trial) : Sample_Tone_StartStop(toneIndex,2,i_trial)), i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(toneIndex+1,1,i_trial) : Sample_Tone_StartStop(toneIndex+1,2,i_trial));
        
        ECoGdata_p1_33(:, 1:length(Sample_Tone_StartStop(1,1,i_trial) : Sample_Tone_StartStop(toneIndex,2,i_trial)), i_trial) = ...
            data_input.trial{i_trial}(:, Sample_Tone_StartStop(1,1,i_trial) : Sample_Tone_StartStop(toneIndex,2,i_trial));
    end
end

%% Test: Additional trial-wise preprocessing for epochs of interest 
%Check single trials for deviations
cfg = [];
cfg.method = 'trial';
% cfg.method = 'channel';
cfg.keepchannel = 'no';
cfg.latency = [data_input.time{1}(Sample_Tone_StartStop(33,1)) data_input.time{1}(Sample_Tone_StartStop(33,2))];%p33
cfg.latency = [data_input.time{1}(Sample_Tone_StartStop(34,1)) data_input.time{1}(Sample_Tone_StartStop(34,2))];%p34
cfg.latency = [data_input.time{1}(Sample_Tone_StartStop(1,1)) data_input.time{1}(Sample_Tone_StartStop(end,2))];%p1-34

test = ft_rejectvisual(cfg,data_input);
