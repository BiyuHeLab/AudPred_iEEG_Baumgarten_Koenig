function NASTD_ECoG_Predict_PlotSignClusterElec_AllSubTD_RegionalFDR...
    (subs, ...
...
%per input data aggregated over subjects and tone durations
%(TD) and projected on 1 Hemisphere.
%Based on sample-wise analysis, thus no time windows but instead entire analysis window.
%With FDR-correction done per anatomical parcel (instead across all electrodes per subject)
...
for i_sub = 1:length(subs)
    ...
    for i_TD = 1:length(FuncInput_ToneDur_text)
        ...
        %Perform regional FDR_correction on cluster-p-values - i.e., select
        %all electrodes within a regional parcel (e.g., a lobe), and then
        %perform FDR-correction for them 
        ...
        %Check which (high-level) anatomical parcels are present
        present_anatparcels = unique(min(AnatReg_allSubs{i_sub, i_TD}.CatIndex,[],2));
        
        for i_anatparcel = 1:length(present_anatparcels)
            
            curr_anatparcel = present_anatparcels(i_anatparcel);
            %AnatReg_allSubs{i_sub, i_TD}.CatLabels{curr_anatparcel};
            filt_curr_anatparcel = ...
                AnatReg_allSubs{i_sub, i_TD}.CatIndex(:,1) == curr_anatparcel;
            index_curr_anatparcel = ...
                find(AnatReg_allSubs{i_sub, i_TD}.CatIndex(:,1) == curr_anatparcel);
            ...
            for i_cluster = 1:size(Param2plot.per_sub.pval{i_sub, i_TD},2)
                [~, cluster_critpFDR,~ , cluster_pvalFDR_perelec(:,i_cluster)] = ...
                    fdr_bh(Param2plot.per_sub.pval{i_sub, i_TD}(filt_curr_anatparcel,i_cluster), param.pval_FDR, 'pdep','no');
            end
            %Replace uncorrected p-value matrix with FDR-corrected p-values
            for i_pval_entry = 1:size(cluster_pvalFDR_perelec,1)
                curr_entrypos = index_curr_anatparcel(i_pval_entry);                
                Param2plot.per_sub.pval{i_sub, i_TD}(curr_entrypos,:) = ...
                    cluster_pvalFDR_perelec(i_pval_entry,:);
            end
        end
