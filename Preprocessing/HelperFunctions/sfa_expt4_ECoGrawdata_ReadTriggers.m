function [info_trigger] = sfa_expt4_ECoGrawdata_ReadTriggers(sub, data_ECoGraw, plot_poststepFigs, save_poststepFigs)
%Function reads out auditory triggers from raw data file, chooses max/first
%marker per deflection as trigger-defining data point and then
%distinguished trigger-types (3 pos. deflections = block start, 2 = trial
%start, 1 = trial end). Optional plotting and saving of process.
    cd
    sfa_expt4_setVars
    paths_sfa_expt4 = sfa_expt4_paths(vars.location);
    sfa_expt4_subjectinfo
...
%1.1) Determine threshold for trigger selection
    ...
    relthresh_level = subs_PreProcSettings.(sub).Triggerthresh; %read out index of label-mismatching channels 
...
%1.2) Determine absolute max/min thresh_level and timepoints of above/below thresh signal
    ...
    index_DPabovemaxthresh = ... %index of data points (relative to .trial) where data points are above thresh
        find(data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,:) >= triggerval_max*relthresh_level);
    TP_DPabovemaxthresh = data_ECoGraw.time{1}... %time point of above thresh data points
        (find(data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,:) >= triggerval_max*relthresh_level));

    triggerval_min = min(data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,:));
    triggerval_minthresh = triggerval_min*relthresh_level;
    index_DPbelowminthresh = find(data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,:) <= triggerval_min*relthresh_level);
    TP_DPbelowminthresh = data_ECoGraw.time{1}...
        (find(data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,:) >= triggerval_min*relthresh_level));
  
% disp([num2str(size(TP_DPabovemaxthresh,2)) ' data points above threshold; ' num2str(size(TP_DPabovemaxthresh,2)) ' data points below threshold']);

%plot trigger chan, max+min thres line, and markers for abovethresh data points
    if plot_poststepFigs == 1
        maxthres_line = [(data_ECoGraw.trial{1}(1,:)./data_ECoGraw.trial{1}(1,:)).*triggerval_maxthresh]; %create row vector for plotting thres line
        minthres_line = [(data_ECoGraw.trial{1}(1,:)./data_ECoGraw.trial{1}(1,:)).*triggerval_minthresh]; %create row vector for plotting thres line
    ...
    figure 
        ...
        if save_poststepFigs == 1            
            path_fig = [paths_sfa_expt4.Fig_ECoGdataraw_SingleSubs sub '/' 'TriggerDef/' ];
            mkdir(path_fig);
            
            filename     = [sub '_TriggerThresh' num2str(relthresh_level*100) '%maxmin_AllTriggerMarker.png'];            
...
%1.3.2 Select first and max data point per deflection (because we either want the want the earliest or max val per trigger deflection)  
for i_neighbours = 1:length(index_DPabovemaxthresh_alldeflections) %loop over above thresh deflections
...
%             data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,index_maxDPabovemaxthresh),['c','.'])    %everything larger than min distance results results in 1 hit per
        title([sub ' TriggerChan WholeRecording TriggerThresh = ' num2str(relthresh_level*100) '% FirstMarkerperDeflection'])
        
        if save_poststepFigs == 1            
            path_fig = [paths_sfa_expt4.Fig_ECoGdataraw_SingleSubs sub '/' 'TriggerDef/' ];
            mkdir(path_fig);
            
            filename     = [sub '_TriggerThresh' num2str(relthresh_level*100) '%maxmin_FirstMarkerperDeflection.png'];            
            figfile      = [path_fig filename];                
    ...
    figure 
        ...
        plot(data_ECoGraw.time{1}(index_maxDPabovemaxthresh), ...
            data_ECoGraw.trial{1}(data_ECoGraw.info_elec.all.index_triggerchan,index_maxDPabovemaxthresh),['c','.'])    %everything larger than min distance results results in 1 hit per
        title([sub ' TriggerChan WholeRecording TriggerThresh = ' num2str(relthresh_level*100) '% MaxValMarkerperDeflection'])
        
        if save_poststepFigs == 1            
            path_fig = [paths_sfa_expt4.Fig_ECoGdataraw_SingleSubs sub '/' 'TriggerDef/' ];
            mkdir(path_fig);
            
            filename     = [sub '_TriggerThresh' num2str(relthresh_level*100) '%maxmin_MaxValMarkerperDeflection.png'];            
            figfile      = [path_fig filename];                
...
%are within a second before/after the next/last trial start trigger
    ...
    %deflection would correlat better with the real tone sequence end. We
    ...
    if plot_poststepFigs == 1
       ...
       title([sub ' FinalTriggerDefinition: ' ...
        ...
        if save_poststepFigs == 1            
            ...
            filename     = [sub '_FinalTriggerDefinition_WholeRecording.png'];            
            figfile      = [path_fig filename];                
...
%Plot blockwise triggerchannel overview of entire recording with all markers    
    ...
    addpath(genpath(paths_sfa_expt4.ScriptsDir));
    subs_PreProcSettings = sfa_expt4_subjectPreProcSettings; %load in file with individual preproc infos
    index_FirstExpBlock = subs_PreProcSettings.(sub).index_FirstExpBlock; %read out index of label-mismatching channels 
            ...
            if i_block < length(index_DP_3deflections)   
                ...
                title([sub ' FinalTriggerDefinition for Block: ' ...
                  ...
                  if save_poststepFigs == 1            
                    ...
                    filename     = [sub '_FinalTriggerDefinition_Block' num2str(i_block-(index_FirstExpBlock-1)) '.png'];            
                    figfile      = [path_fig filename];                
...
for i_block = 1:length(info_trigger.Exp.BlockStart_index)
    ...
    if ~isempty(data.redoneTrialNums{i_block})
        for i_repeatedtrial = 1:length(data.redoneTrialNums{i_block})
            if length(data.missedTrialNums{i_block}) == 1 %When only one trial was misses (even multiple times)
                data_behav_trialtonedur_perblock{i_block}(end+1) = data.stim.toneDur(data.missedTrialNums{i_block}(1));
            else %when multiple trials were missed
                data_behav_trialtonedur_perblock{i_block}(end+1) = data.stim.toneDur(data.missedTrialNums{i_block}(i_repeatedtrial));
            end
    ...
    for i_trial = 1:length(info_trigger.Exp.TrialStart_index{i_block})
        ...
        elseif info_trigger.Exp.TrialEnd_TP{i_block}(i_trial) - ... %if trigger-def trialdur is long (larger than 10s)
           ...
           data_behav_trialtonedur_perblock{i_block}(i_trial) ~= 0.4 %and stim-def trialdur is not short
           disp(['TrialDur mismatch in Block: ' num2str(i_block) ', Trial: ' num2str(i_trial)])
        end
    end
end
