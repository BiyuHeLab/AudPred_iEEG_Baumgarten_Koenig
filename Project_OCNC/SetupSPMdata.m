...
for i_chan = 1:length(data_ECoG.label)
    temp_hdr.chantype{i_chan} = 'other';
end
...
% 5) Downsample to 50 Hz (since for ERP we don't need higher freqs)
%original: 512 Hz - cycle length = 0.002 s - 100/200 samples per 200/400 ms tone
%original: 50 Hz - cycle length = 0.02 s - 10/20 samples per 200/400 ms tone
% cfg = [];
% cfg.resamplefs = 50;
% data_ECoG = ft_resampledata(cfg,data_ECoG);
% nSamplesPerTone = toneDur_inSecs * fsample;

% %% 2) Save data
% %2.1 FT format
% savefile_matlab = [path_save sub '_data_' tonedur_title 'ms_matlab.mat'];
% save(savefile_matlab, 'data_ECoG','-v7.3');
% 
%  %2.2 SPM format
% addpath('D:\Programs\MATLAB\spm12\') %path SPM
% savefile_spm = [path_save sub '_data_' tonedur_title 'ms.mat'];
% data_ECoG_SPM = spm_eeg_ft2spm(data_ECoG, savefile_spm);
   

%% 3) Reduce data set to only ROI channels and likely vs unlikely trials
%3.1 Determine trial groups (i.e., large vs. small p*34-p34 distance)
...
cd '/isilon/LFMI/VMdrive/Thomas/toolboxes/fieldtrip-20190314/utilities/'
cfg = [];
...
data_ECoG_reduced.sampleinfo = data_ECoG.sampleinfo(sort([f_likely f_unlikely]),:);
...
%Plot ERP
figure;
plot(1:length(ERP_likely.avg), ERP_likely.avg)
legend('A1','STG','IFG')
figure;
