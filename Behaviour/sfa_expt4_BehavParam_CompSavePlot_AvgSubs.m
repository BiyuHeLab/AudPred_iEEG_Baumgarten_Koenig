...
path_fig = paths_sfa_expt4.Fig_Behavior_GroupAvg;

i_sub = 1;
subs = vars.sub_list; %patients
sub = subs{i_sub};
% subs = {'NY704'}; %patients - No N&688, since different paradigm vars 
% subs = {'TestThomas', 'TestLark','TestElla','TestJonathan'}; %healthy test subjects normal task2
% subs = {'TestThomas_easytask2'}; %healthy test subjects easy task2
...
BehavData_GroupAvg.FTPLrating_stats = sfa_expt4_BehavParam_ANOVAs_AvgSubs(subs, tonedur_text, location);
...
%8.1 plot final tone probability rating independent of and dependent on math. expected tone
% xlab = {'539' '586' '655'};
...
mkdir([paths_sfa_expt4.Analysis_Behavior 'GroupAvg/'])
save([paths_sfa_expt4.Analysis_Behavior GroupAvg '/' 'BehavData_Group.mat'], 'BehavData_GroupAvg')
